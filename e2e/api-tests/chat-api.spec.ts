...
const API_BASE = process.env.VITE_API_URL || "http://localhost:3004";
...
test.describe("Chat API", () => {
  test.skip(
    !LIVE_API_E2E,
    "Set LIVE_API_E2E=true with a seeded test API to run this live suite",
  );
  ...
  test.describe("POST /chat/{id}/ai/invoke - AI Invocation", () => {
    test("invokes AI and gets response", async ({ request }) => {
      ...
      // Invoke AI
      const aiResponse = await request.post(
        `${API_BASE}/chat/${chatId}/ai/invoke`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${authToken}`,
    ...
    test("handles invalid chat ID", async ({ request }) => {
      const response = await request.post(
        `${API_BASE}/chat/invalid-id/ai/invoke`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${authToken}`,
    ...
    test("requires message content", async ({ request }) => {
      const response = await request.post(
        `${API_BASE}/chat/some-id/ai/invoke`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${authToken}`,
  ...
  test.describe("Rate Limiting", () => {
    test("enforces rate limits on AI invocation", async ({ request }) => {
      ...
      for (let i = 0; i < 5; i++) {
        const response = await request.post(
          `${API_BASE}/chat/test-chat/ai/invoke`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authToken}`,
  ...
  test.describe("Error Handling", () => {
    test("returns proper error format", async ({ request }) => {
      const response = await request.post(
        `${API_BASE}/chat/nonexistent/ai/invoke`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${authToken}`,
