...
export async function requestOtpCode(
  ...
  if (!data.data.code) {
    throw new Error(
      "OTP code not returned - ensure DENO_ENV=development on backend",
    );
  }
 ...
 */
export async function getAllTestUserTokens(): Promise<
  Record<string, AuthResult>
> {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  async function main() {
    ...
    if (seeded) {
      ...
      console.log(
        "Run: psql -U postgres -d juicyvision -f e2e/fixtures/seed-test-users.sql",
      );
    }
    console.log();
