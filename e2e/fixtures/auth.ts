...
const tokenCache = new Map<
  string,
  { token: RealTestUser; timestamp: number }
>();
  ...
  const authPromise = (async () => {
    try {
      ...
      if (!codeData.success || !codeData.data.code) {
        throw new Error(
          `Failed to get OTP code: ${codeData.error || "No code returned (is backend in dev mode?)"}`,
        );
      }
 ...
 */
export async function mockExternalWallet(
  page: Page,
  options: {
    address?: string;
    chainId?: number;
 ...
 */
export async function mockSIWE(
  page: Page,
  options: {
