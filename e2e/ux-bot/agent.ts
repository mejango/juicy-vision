...
export class UXTestAgent {
      ...
      try {
        ...
        for (const issue of analysis.uxIssues) {
          if (!state.issues.some((i) => i.id === issue.id)) {
            ...
            console.log(
              `[UX Bot] Issue found: [${issue.severity}] ${issue.title}`,
            );
          }
        }
        ...
        if (this.config.stopOnCriticalIssue) {
          const criticalIssue = analysis.uxIssues.find(
            (i) => i.severity === "critical",
          );
          if (criticalIssue) {
            console.log(
              `[UX Bot] Stopping due to critical issue: ${criticalIssue.title}`,
            );
        ...
        if (action && !state.isComplete) {
          console.log(
            `[UX Bot] Action: ${action.type} - ${action.description || ""}`,
          );
          ...
          previousActions.push(
            `${action.type}: ${action.description || JSON.stringify(action)}`,
          );
        ...
        if (state.currentStep >= 5 && analysis.progress < 10) {
          console.log(
            "[UX Bot] Progress seems stuck, continuing exploration...",
          );
        }
      } catch (error) {
  ...
  private generateSummary(state: AgentState, steps: TestStep[]): string {
    ...
    if (state.issues.length > 0) {
      ...
      parts.push(
        `Found ${state.issues.length} issues: ${critical} critical, ${major} major, ${minor} minor/suggestions.`,
      );
    } else {
  ...
  private generateRecommendations(issues: UXIssue[]): string[] {
    ...
    if (byCategory.functionality?.length) {
      recommendations.push(
        "Address functionality issues first as they may block user flows.",
      );
    }
    ...
    if (byCategory.usability?.length) {
      recommendations.push(
        "Consider user testing to validate UX improvements.",
      );
    }

    if (byCategory.visual?.length) {
      recommendations.push(
        "Review visual design for consistency across the app.",
      );
    }
