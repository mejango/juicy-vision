...
export class APITestClient {
  ...
  constructor(
    baseUrl: string = process.env.VITE_API_URL || "http://localhost:3004",
  ) {
      ...
      if (!response.ok) {
        result.error =
          typeof responseData === "object" && responseData !== null
            ? (responseData as Record<string, unknown>).error?.toString() ||
              "Request failed"
            : "Request failed";
      }
  ...
  static printResults(suites: APITestSuite[]) {
    ...
    for (const suite of suites) {
      ...
      for (const result of suite.results) {
        ...
        console.log(
          `  ${icon} [${status}] ${result.method} ${result.endpoint} (${result.responseTime}ms)`,
        );
