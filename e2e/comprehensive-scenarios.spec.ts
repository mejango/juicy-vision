...
async function ensureSeeded() {
  ...
  await seedTestUsers();
  try {
    await fetch("http://localhost:3004/debug/seed-full-test-data", {
      method: "POST",
    });
  } catch {
    console.log(
      "Could not seed full test data - some tests may have limited data",
    );
  }
}
  ...
  for (const scenario of ALL_COMPREHENSIVE_SCENARIOS.ruleset.main) {
    authTest(
      ...
      async ({ page, realAuth }) => {
        ...
        console.log(
          `[Ruleset] ${report.status} - ${report.steps.length} steps, ${report.issues.length} issues`,
        );
      },
    );
  }
  ...
  for (const scenario of ALL_COMPREHENSIVE_SCENARIOS.ruleset.edge) {
    authTest(
      ...
      async ({ page, realAuth }) => {
        ...
        const report = await agent.runScenario(scenario);
        console.log(
          `[Ruleset Edge] ${report.status} - ${report.issues.length} issues`,
        );
      },
    );
  }
  ...
  for (const scenario of ALL_COMPREHENSIVE_SCENARIOS.nftTier.main) {
    authTest(
      ...
      async ({ page, realAuth }) => {
        ...
        const report = await agent.runScenario(scenario);
        console.log(
          `[NFT Tier] ${report.status} - ${report.issues.length} issues`,
        );
      },
    );
  }
...
authTest.describe("Comprehensive - Full Suite Summary", () => {
  ...
  authTest(
    "runs ALL main scenarios and generates report",
    async ({ page, realAuth }) => {
      authTest.skip(
      ...
      for (const result of results) {
        console.log(
          `[${result.status.toUpperCase()}] ${result.scenario}... (${result.issues} issues)`,
        );
      }
    },
  );
