 ...
 */
const BANNY_LP_SPLIT_HOOK: Address = '0xae6705c33c8b46f56878a1d4f1ce4d75fcfb6f62'
const BANNY_LP_SPLIT_HOOK_CHAINS = new Set([1, 10, 8453, 42161, 11155111, 84532, 421614])
...
const positionViewAbi = [
  ...
  },
  {
    ...
    inputs: [{ name: 'id', type: 'uint256' }], outputs: [{ name: 'owner', type: 'address' }],
  },
] as const
...
const buybackRegistryAbi = [{
  type: 'function', name: 'hookOf', stateMutability: 'view',
  inputs: [{ name: 'projectId', type: 'uint256' }], outputs: [{ type: 'address' }],
}] as const

const omnichainExtraHookAbi = [{
  type: 'function', name: 'extraDataHookOf', stateMutability: 'view',
  inputs: [
    { name: 'projectId', type: 'uint256' },
...
export function lpHookErrorText(message: string, symbol: string): string | null {
  ...
  if (/NoTokensAccumulated/.test(m)) return `No reserved ${symbol} has accumulated here yet.`
...
async function projectDataHook(chainId: number, projectId: bigint, client: PublicClient): Promise<DataHookInfo | null> {
  try {
...
export async function resolveBuybackHook(
  ...
  const recognize = async (dataHook: Address | null): Promise<Address | null> => {
...
export async function readPoolState(chainId: number, projectId: bigint): Promise<PoolState | null> {
...
export function collectPoolLogs(logs: readonly RawLog[], positionManager: Address, state: PoolLogScanState): void {
  ...
  for (const log of logs || []) {
    ...
    if (topic0 === LP_INITIALIZE_TOPIC) {
      if (state.initializeBlock == null || block < state.initializeBlock) state.initializeBlock = block
      continue
    }
...
async function fetchPoolLogsRange(
  ...
  const topics = [
    onlyInitialize ? LP_INITIALIZE_TOPIC : [LP_INITIALIZE_TOPIC, LP_MODIFY_LIQUIDITY_TOPIC],
    poolId,
  ]
...
export async function scanPoolPositionTokenIds(
  ...
  const state: PoolLogScanState = { initializeBlock: cached?.initializeBlock ?? null, tokenIds: new Map() }
  let complete = cached ? cached.complete : true
  let windowsUsed = 0

  const scanRange = async (fromBlock: bigint, toBlock: bigint): Promise<void> => {
    ...
    while (cursor <= toBlock) {
      ...
      try {
        const batches = await Promise.all(
          ranges.map(range => fetchPoolLogsRange(client, poolManager, poolId, range.lo, range.hi, false)),
        )
        for (const logs of batches) collectPoolLogs(logs, positionManager, state)
      } catch {
        complete = false
        return
      }
  ...
  if (cached && state.initializeBlock != null && latest >= state.initializeBlock) {
    // Incremental: keep ids minted before the reorg-overlap tail, rescan the tail.
    let overlapStart =
      cached.throughBlock > LP_LOG_REORG_OVERLAP ? cached.throughBlock - LP_LOG_REORG_OVERLAP + 1n : cached.initializeBlock
    if (overlapStart < cached.initializeBlock) overlapStart = cached.initializeBlock
    for (const entry of cached.entries) {
      if (entry.block < overlapStart) state.tokenIds.set(entry.id.toString(), entry)
    }
    ...
    try {
      const initializeLogs = await fetchPoolLogsRange(client, poolManager, poolId, 0n, latest, true)
    ...
    if (state.initializeBlock != null) {
      ...
      while (windowsUsed < LP_LOG_MAX_WINDOWS && cursor >= 0n) {
        const ranges: Array<{ lo: bigint; hi: bigint }> = []
        for (let n = 0; n < LP_LOG_BATCH_WINDOWS && windowsUsed < LP_LOG_MAX_WINDOWS; n++) {
  ...
  const tokenIds = entries.map(entry => entry.id).sort((a, b) => (a < b ? -1 : a > b ? 1 : 0))
  if (state.initializeBlock != null) {
...
export async function readPositionDetails(
  ...
  const details = await Promise.all(
    tokenIds.map(async tokenId => {
      const [info, ownerResult, liquidity] = await Promise.all([
        client
          .readContract({ address: positionManager, abi: positionViewAbi, functionName: 'positionInfo', args: [tokenId] })
          .then(BigInt),
        client
          .readContract({ address: positionManager, abi: positionViewAbi, functionName: 'ownerOf', args: [tokenId] })
          .then(owner => ({ owner: owner as Address, error: null as unknown }))
          .catch(error => ({ owner: null, error })),
        client
          .readContract({ address: positionManager, abi: positionViewAbi, functionName: 'getPositionLiquidity', args: [tokenId] })
          .then(BigInt),
      ])
      // ownerOf legitimately reverts after an NFT is burned. Any owner failure
      // for a live position is incomplete data.
      if (ownerResult.error && info !== 0n && liquidity !== 0n) throw ownerResult.error
  ...
  await Promise.all(
    details.map(async detail => {
      ...
      if (BigInt(info) !== detail.info) throw new Error('PositionManager returned inconsistent LP position data')
      if (poolIdOf(poolKey as PoolKey).toLowerCase() !== expectedPoolId.toLowerCase()) {
        throw new Error('PositionManager token does not belong to the expected pool')
      }
    }),
  )
  return details
 ...
 */
export async function readLpPositions(chainId: number, projectId: bigint): Promise<LpPositionsSnapshot | null> {
  ...
  const scan = await scanPoolPositionTokenIds(chainId, poolManager, positionManager, state.poolId)
  ...
  const details = await readPositionDetails(client, positionManager, scan.tokenIds, state.poolId)
  ...
  for (const detail of details) {
    ...
    const amounts = lpGetAmountsForLiquidity(state.sqrtP, lpSqrtAtTick(tickLower), lpSqrtAtTick(tickUpper), detail.liquidity)
    ...
    positions.push({ tickLower, tickUpper, liquidity: detail.liquidity, pairAmount, tokenAmount })
    const value = Number(pairAmount) / pairScale + (Number(tokenAmount) / 1e18) * state.poolPrice
  ...
  return { state, owners, positions, totalPair, totalToken, complete: scan.complete }
}
...
async function scanPoolPositionsViaHook(
  ...
  const positionManager = POSITION_MANAGER_BY_CHAIN[chainId]
  const poolManager = POOL_MANAGER_BY_CHAIN[chainId]
  if (!positionManager || !poolManager || !hook || hook === zeroAddress) return []
  const client = clientFor(chainId)
  const state = await readPoolStateViaHook(chainId, projectId, hook, client).catch(() => null)
  if (!state) return []
  const scan = await scanPoolPositionTokenIds(chainId, poolManager, positionManager, state.poolId)
...
export async function readUserLpPositions(
  ...
  if (currentHook) tasks.push(scanPoolPositionsViaHook(chainId, projectId, currentHook, true))
  if (lc(JB_BUYBACK_HOOK) !== lc(currentHook)) {
    tasks.push(scanPoolPositionsViaHook(chainId, projectId, JB_BUYBACK_HOOK, false))
  }
...
export async function refreshLpPosition(
  ...
  const positionManager = POSITION_MANAGER_BY_CHAIN[chainId]
  const poolManager = POOL_MANAGER_BY_CHAIN[chainId]
  if (!positionManager || !poolManager) throw new Error('Liquidity contracts are unavailable on this chain.')
  const client = clientFor(chainId)
  const [details, sqrtP] = await Promise.all([
    readPositionDetails(client, positionManager, [position.tokenId], position.poolId),
    readSlot0SqrtPrice(client, poolManager, position.poolId),
  ])
  const detail = details[0]
  if (!detail.owner || detail.owner.toLowerCase() !== expectedAccount.toLowerCase()) {
  ...
  const { tickLower, tickUpper } = ticksFromPositionInfo(detail.info)
  const amounts = lpGetAmountsForLiquidity(sqrtP, lpSqrtAtTick(tickLower), lpSqrtAtTick(tickUpper), detail.liquidity)
  return {
    ...position,
    owner: detail.owner,
    liquidity: detail.liquidity,
...
export function deriveAddLiquidityPlan(opts: {
  ...
  const pairIsC0 = (key.currency0 || '').toLowerCase() === pair.addr.toLowerCase()
  ...
  const pRawFromQ = (q: number) => (pairIsC0 ? 10 ** (18 - pairDec) / q : q * 10 ** (pairDec - 18))
  const tA = Math.log(pRawFromQ(opts.pa)) / Math.log(1.0001)
  const tB = Math.log(pRawFromQ(opts.pb)) / Math.log(1.0001)
  let tickLower = Math.max(minUsable, lpAlignDown(Math.floor(Math.min(tA, tB)), spacing))
  let tickUpper = Math.min(maxUsable, lpAlignUp(Math.ceil(Math.max(tA, tB)), spacing))
  if (tickUpper <= tickLower) tickUpper = Math.min(maxUsable, tickLower + spacing)
  const curTick = Math.floor((2 * Math.log(Number(sqrtP) / 2 ** 96)) / Math.log(1.0001))
  if (amount1 <= 0n && amount0 > 0n && curTick >= tickLower) tickLower = Math.min(maxUsable, lpAlignUp(curTick + 1, spacing))
  if (amount0 <= 0n && amount1 > 0n && curTick < tickUpper) tickUpper = Math.max(minUsable, lpAlignDown(curTick, spacing))
  if (tickUpper <= tickLower) tickUpper = Math.min(maxUsable, tickLower + spacing)

  const sqrtA = lpSqrtAtTick(tickLower)
  const sqrtB = lpSqrtAtTick(tickUpper)
  const liquidity = lpGetLiquidityForAmounts(sqrtP, sqrtA, sqrtB, amount0, amount1)
  ...
  if (!c0Native && amount0Max > 1n) erc20Sides.push({ currency: key.currency0, max: amount0Max })
  if (!c1Native && amount1Max > 1n) erc20Sides.push({ currency: key.currency1, max: amount1Max })
  ...
  let actions = `0x${ACTION_MINT_POSITION}${ACTION_CLOSE_CURRENCY}${ACTION_CLOSE_CURRENCY}` as `0x${string}`
  if (pair.isNative) {
    ...
    parts.push(encodeAbiParameters([{ type: 'address' }, { type: 'address' }], [nativeCurrency, recipient]))
    actions = `0x${ACTION_MINT_POSITION}${ACTION_CLOSE_CURRENCY}${ACTION_CLOSE_CURRENCY}${ACTION_SWEEP}` as `0x${string}`
  }
  const unlockData = encodeAbiParameters([{ type: 'bytes' }, { type: 'bytes[]' }], [actions, parts])
...
export async function prepareAddLiquidity(opts: {
  ...
  if (!POSITION_MANAGER_BY_CHAIN[opts.chainId]) throw new Error('No position manager on this chain')
 ...
 * more than reviewed.
 */
export function assertPlanWithinCorridor(plan: AddLiquidityPlan, freshSqrtP: bigint): void {
  if (freshSqrtP <= 0n) throw new Error('Could not verify the current pool price')
  const sqrtA = lpSqrtAtTick(plan.tickLower)
  const sqrtB = lpSqrtAtTick(plan.tickUpper)
  const need = lpGetAmountsForLiquidity(freshSqrtP, sqrtA, sqrtB, plan.liquidity)
  if (need.amount0 > plan.amount0Max || need.amount1 > plan.amount1Max) {
    throw new Error('The pool price moved while you were reviewing. Reopen and review the updated amounts.')
  }
}

/** Reverify helper: re-read slot0 for the plan's pool and run the corridor check. */
export async function reverifyAddLiquidity(chainId: number, plan: AddLiquidityPlan): Promise<void> {
  const poolManager = POOL_MANAGER_BY_CHAIN[chainId]
  if (!poolManager) throw new Error('Liquidity contracts are unavailable on this chain.')
  const freshSqrtP = await readSlot0SqrtPrice(clientFor(chainId), poolManager, poolIdOf(plan.key))
...
export function prepareRemoveLiquidity(
  position: Pick<UserLpPosition, 'tokenId' | 'key' | 'pairAmount' | 'tokenAmount' | 'pairIsC0'>,
  recipient: Address,
  nowSeconds: number = Math.floor(Date.now() / 1000),
): RemoveLiquidityPlan {
...
export async function readCashOutFloorPrice(
  ...
  try {
    ...
    const reclaim = await client.readContract({
      ...
      args: [projectId, ONE_TOKEN, [], [token], BigInt(pair.decimals), currency],
    })
...
export async function readMarketRulesetFacts(chainId: number, projectId: bigint): Promise<MarketRulesetFacts | null> {
...
export async function projectUsesLpSplitHook(chainId: number, projectId: bigint): Promise<boolean> {
  ...
  try {
    ...
    return splits.some(split => (split.hook || '').toLowerCase() === hookAddress.toLowerCase())
  } catch {
    return false
  }
}
...
export async function readSplitHookState(
  ...
  const read = <T,>(functionName: string, args: readonly unknown[]): Promise<T | null> =>
    (client.readContract({
      address: hookAddress,
      abi: bannyLpSplitHookAbi,
      functionName,
      args,
    } as never) as Promise<T>).catch(() => null)

  const [accumulated, hasPool, fees, initialWeight, tokenId, tickLower, tickUpper] = await Promise.all([
