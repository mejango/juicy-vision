...
function ensureListener(): void {
  ...
  window.addEventListener("message", (event) => {
    ...
    if (msg.success === false || msg.error) {
      p.reject(
        new Error(
          typeof msg.error === "string" ? msg.error : "Safe request failed",
        ),
      );
    } else {
      p.resolve(msg.data);
...
function safeRpc<T = unknown>(method: string, params?: unknown): Promise<T> {
  ensureListener();
  if (!inIframe())
    return Promise.reject(new Error("Not running inside Safe{Wallet}."));
  const id = `jb-safe-${++idCounter}-${Date.now()}`;
  const message = {
    id,
    method,
    params: params ?? {},
    env: { sdkVersion: SDK_VERSION },
  };
 ...
 */
export function detectSafeApp(timeoutMs = 10_000): Promise<SafeInfo | null> {
 ...
 */
export function txHashForSafeTx(
  safeTxHash: string,
): Promise<`0x${string}` | null> {
  return safeRpc<{ txHash?: `0x${string}` }>("getTxBySafeTxHash", {
    safeTxHash,
  })
...
export function encodeApprovalTx(
  token: `0x${string}`,
  spender: `0x${string}`,
  amount: bigint,
): SafeTx {
    ...
    request(args: { method: string; params?: unknown[] }): Promise<unknown> {
      ...
      switch (method) {
        ...
        case "wallet_switchEthereumChain": {
          ...
          if (want && String(want).toLowerCase() === chainIdHex.toLowerCase())
            return Promise.resolve(null);
        ...
        case "eth_sendTransaction": {
          ...
          return safeRpc<{ safeTxHash?: `0x${string}` }>("sendTransactions", {
            ...
            if (!r?.safeTxHash)
              throw new Error("Safe returned no proposal hash.");
            // EIP-1193 callers expect eth_sendTransaction to return a mined
            // transaction hash they may safely receipt-poll. Safe returns a
            // proposal identifier instead, so resolve it before exposing it
            // through the compatibility provider.
            return waitForSafeExecution(r.safeTxHash);
          });
        }
        case "personal_sign":
          return safeRpc<{ signature?: string; safeTxHash?: string }>(
            "signMessage",
            {
              message: params[0],
            },
