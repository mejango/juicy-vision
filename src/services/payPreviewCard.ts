...
export function resolveDirectSwapOffer(params: {
  ...
  const build = (poolId: Hex, quotedOut: bigint): DirectSwapOffer => ({
    poolId,
    quotedOut,
    minOut: quotedOutputFloor(
      quotedOut,
      10_000n - (params.slippageBps ?? 100n),
    ),
...
/** Human routing label for the pay preview chip. */
export function formatRoutingTag(
  route: "issuance" | "amm",
): "Swap" | "Issuance" {
  return route === "amm" ? "Swap" : "Issuance";
}

/** The buyer-facing discount label overlaid on a shop tier ("25% off"), denominator 200. */
export function tierDiscountLabel(
...
export function computeNftCheckout(params: {
  ...
  const restrictedCost =
    params.restrictedCost < 0n ? 0n : params.restrictedCost;
...
export function formatExactTokenEstimate(
  value: bigint,
