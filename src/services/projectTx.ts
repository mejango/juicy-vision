...
async function ensureWalletOnChain(
  ...
  if ((await ctx.walletClient.getChainId()) !== chainId) {
    throw new Error(
      `Wallet did not switch to ${CHAINS[chainId]?.name ?? `chain ${chainId}`}`,
    );
  }
}
...
async function sendRaw(
  ...
  if (ctx.isManagedMode) {
    return (await executeManagedTransaction(
      chainId,
      to,
      data,
      value.toString(),
  ...
  if (
    !account ||
    account.address.toLowerCase() !== ctx.activeAddress.toLowerCase()
  ) {
    throw new Error(
 ...
 */
export async function runGuardedTx(
  ctx: GuardedWalletContext,
  ...
  await requireTransactionReview({
    ...
    title:
      req.review?.title ??
      (ctx.isSafeMode ? "Review Safe proposal" : "Review transaction"),
    description:
      req.review?.description ??
      (ctx.isSafeMode
        ? "You are proposing this exact batch to Safe. On Safe’s confirmation screen, Nonce defaults to the next available value and the dropdown lists queued nonces if you want to replace one. The proposal remains pending until the required owners approve and execute it."
        : "Review every call below before your wallet is opened."),
    confirmLabel: ctx.isSafeMode
  ...
  const updateActivity = (
    updates: Parameters<typeof store.updateTransaction>[1],
  ...
  try {
    ...
    if (req.approval) {
      ...
      if (current < amount) {
        ...
        const duplicateApproval = useTransactionStore
          .getState()
          .transactions.find(
            (tx) =>
        ...
        const approvalActivityId = useTransactionStore
          .getState()
          .addTransaction({
            type: "contractCall",
            chainId,
        ...
        if (after < amount)
          throw new Error("The token approval did not take effect");
      }
    }
    ...
    if (safeProposalHash && !submittedHash && !cancelled) {
      updateActivity({
        ...
        stage: undefined,
        error:
          "The Safe proposal is still awaiting execution or its status is temporarily unavailable. Do not submit it again.",
      });
      throw error;
    }
    updateActivity({
