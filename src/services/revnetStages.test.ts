...
describe("decodeStageMetadata", () => {
  ...
  it("decodes an all-zero metadata word", () => {
    expect(decodeStageMetadata(0n)).toEqual({
      reservedPercent: 0,
      cashOutTaxRate: 0,
      baseCurrency: 0,
    });
...
describe("issuanceAtTime", () => {
  ...
  it("treats duration 0 as perpetual with no cuts", () => {
    const stages = [
      stage({ start: 0, duration: 0, weight, weightCutPercent: 100_000_000 }),
    ];
  ...
  it("switches to the active stage at its start and restarts that stage's cut clock", () => {
    const stages = [
      ...
      stage({
        ...
        weightCutPercent: 380_000_000,
      }),
    ];
    expect(issuanceAtTime(stages, 100 * DAY - 1)).toBeCloseTo(
      1000 * 0.9 ** 99,
      6,
    );
...
describe("stage helpers", () => {
  ...
  it("nextCutAt returns the upcoming boundary, or null when the stage never cuts", () => {
    ...
    expect(
      nextCutAt(
        stage({ start: 100, duration: 0, weightCutPercent: 100_000_000 }),
        120,
      ),
    ).toBeNull();
    expect(
      nextCutAt(stage({ start: 100, duration: 50, weightCutPercent: 0 }), 120),
    ).toBeNull();
  });
});
...
describe("formatting", () => {
  it("formatIssuanceRate matches the website tiers", () => {
    ...
    expect(formatIssuanceRate(1234.5)).toBe("1,234.50");
    expect(formatIssuanceRate(9999.90504)).toBe("9,999.91");
