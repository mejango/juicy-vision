...
import {
  ...
  type OwnersSubtabId,
  type ProjectTabId,
} from "../components/project/flavor";
...
export default function ProjectDashboard({
  ...
  const [activeTab, setActiveTabState] = useState<ProjectTabId>(
    initialHash.tab ?? "overview",
  );
  const [ownersSubtab, setOwnersSubtabState] = useState<OwnersSubtabId>(
    initialHash.subtab ?? "accounts",
  );

  // The hash is rewritten by both setters, so the flavor slug (owner/operator) stays right.
  const isRevnetRef = useRef(false);
  const writeHash = useCallback(
    (tab: ProjectTabId, subtab: OwnersSubtabId | null) => {
      window.history.replaceState(
        null,
        "",
        projectHashFor(tab, isRevnetRef.current, subtab),
      );
    },
    [],
  );
  ...
  const setActiveTab = useCallback(
    (tab: ProjectTabId) => {
      ...
      writeHash(
        tab,
        tab === "owners" || tab === "tokens" ? ownersSubtab : null,
      );
    },
    [ownersSubtab, writeHash],
  );

  const setOwnersSubtab = useCallback(
    (id: OwnersSubtabId) => {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("juice:switch-tab", handleSwitchTab);
  }, [setActiveTab]);
  ...
  const [suckerGroupBalance, setSuckerGroupBalance] =
    useState<SuckerGroupBalance | null>(null);
  const [revnetOperator, setRevnetOperator] = useState<string | null>(null);
  const [revnetOperatorError, setRevnetOperatorError] = useState<string | null>(
    null,
  );
  const [displayAddressEns, setDisplayAddressEns] = useState<string | null>(
    null,
  );
  ...
  const isOwner = useMemo(() => {
    if (!currentUserAddress || !project?.owner || project.configurationError)
      return false;
    const authority = isRevnetProject(project) ? revnetOperator : project.owner;
    return (
      !!authority &&
      currentUserAddress.toLowerCase() === authority.toLowerCase()
    );
  }, [currentUserAddress, project, revnetOperator]);

  // Check if project has deployed an ERC20 token
  const hasErc20Token = Boolean(project?.tokenSymbol);

  const balanceAvailable =
    !!suckerGroupBalance && suckerGroupBalance.balanceAvailable !== false;
  const paymentsAvailable =
    !!suckerGroupBalance && suckerGroupBalance.paymentsAvailable !== false;
  ...
  const balanceScope =
    suckerGroupBalance?.dataScope === "project" ? " on this chain" : "";

  const displayBalance = balanceAvailable
    ? suckerGroupBalance.totalBalance
    : "";
  const displayPaymentsCount = paymentsAvailable
    ? suckerGroupBalance.totalPaymentsCount
    : Number.NaN;
    ...
    async function loadDisplayInfo() {
      ...
      if (projectIsRevnet) {
        ...
        try {
          ...
          setDisplayAddressEns(
            operator ? await resolveEnsName(operator) : null,
          );
        } catch (error) {
          setRevnetOperator(null);
          setDisplayAddressEns(null);
          setRevnetOperatorError(
            error instanceof Error
              ? error.message
              : "Revnet operator unavailable",
          );
        }
      } else {
      ...
      <ProjectTabs
        ...
        shop={
          <ShopTab
            ...
            isOwner={isOwner}
            isRevnet={projectIsRevnet}
            connectedChains={connectedChains.map((c) => ({
              chainId: c.chainId,
  ...
  // Owner-menu sections/actions, wired once. `show` gates the whole section (Inventory
  // needs an NFT hook) or a single item (Deploy ERC20 hides once one exists).
  const OWNER_ACTIONS: Array<{
    section: string;
    show?: boolean;
    items: Array<{
      label: string;
  ...
  }> = [
    {
      section: "Funds",
      items: [
        { label: "Send Payouts", modal: "payouts" },
        { label: "Use Surplus Allowance", modal: "surplusAllowance" },
      ],
    },
    {
      section: "Tokens",
      items: [
        { label: "Send Reserved Tokens", modal: "reservedTokens" },
        { label: "Deploy ERC20", modal: "deployErc20", show: !hasErc20Token },
      ],
    },
    {
      section: "Configuration",
      items: [
        { label: "Queue Ruleset", modal: "ruleset" },
        { label: "Configure Splits", modal: "setSplits" },
        { label: "Update Metadata", modal: "setUri" },
      ],
    },
    {
      section: "Inventory",
      show: hasNftHook,
      items: [{ label: "Manage NFT Tiers", modal: "manageTiers" }],
    },
  ];

  /**
   * The project header (logo/name/stats/operator-owner/site + owner menu), wired
   * once for both layouts. Only the size tokens and the menu alignment differ; the
   * desktop-only stopPropagation is kept in the shared helper (a no-op on mobile).
   */
  const renderHeader = (mobile: boolean) => {
    if (!project) return null;
    const logoSize = mobile ? "w-14 h-14" : "w-16 h-16";
    const logoText = mobile ? "text-xl" : "text-2xl";
    const menuAlign = mobile ? "right-0" : "left-0";
    const statsRow = mobile
      ? `flex flex-wrap gap-x-3 gap-y-1 mt-1 text-xs ${isDark ? "text-gray-400" : "text-gray-500"}`
      : `flex items-center gap-4 mt-1 text-sm ${isDark ? "text-gray-300" : "text-gray-600"}`;
    return (
      <div className={mobile ? "px-4 py-4" : "px-6 py-4"}>
        <button
        ...
        <div className={`flex items-start ${mobile ? "gap-3" : "gap-4"}`}>
          ...
          {project.logoUri ? (
            <IpfsImage
              ...
              fallback={
                <div
                  className={`${logoSize} flex items-center justify-center ${logoText} ${isDark ? "bg-white/10" : "bg-gray-200"}`}
                >
                  ?
                </div>
              }
            ...
            <div
              className={`${logoSize} flex items-center justify-center ${logoText} ${
          ...
          <div className="flex-1 min-w-0">
            <h1
              className={`${mobile ? "text-lg" : "text-xl"} font-semibold truncate ${
            ...
            <div className={statsRow}>
              ...
              >
                <span
                  className={`font-semibold cursor-pointer ${isDark ? "text-white" : "text-gray-900"}`}
                >
                  {balanceAvailable
                ...
                {showBalanceTooltip &&
                  balanceAvailable &&
                  suckerGroupBalance &&
                  suckerGroupBalance.projectBalances.length > 1 && (
                    <div
                      className={`absolute top-full left-0 mt-1 p-2 shadow-lg z-50 min-w-[200px] text-xs ${
                        isDark
                          ? "bg-juice-dark border border-white/20"
                          : "bg-white border border-gray-200"
                      }`}
                    >
                      {suckerGroupBalance.projectBalances.map((pb) => {
                        const chainName =
                          (CHAINS[pb.chainId] || MAINNET_CHAINS[pb.chainId])
                            ?.name || `Chain ${pb.chainId}`;
                        const pbCurrency =
                          pb.currency ?? suckerGroupBalance.currency;
                        const pbDecimals =
                          pb.decimals ?? suckerGroupBalance.decimals;
                        return (
                          <div
                            key={pb.chainId}
                      ...
                      >
                        ...
                        >
                          {formatBalanceNative(
                            displayBalance,
                            suckerGroupBalance?.currency,
                            suckerGroupBalance?.decimals,
                          )}
                        </span>
                      </div>
                    </div>
              ...
              <span>
                ...
                payment
                {!paymentsAvailable || displayPaymentsCount !== 1 ? "s" : ""}
              </span>
            </div>
            ...
            {(displayAddress ||
              ...
              <div
                className={`flex items-center flex-wrap gap-x-1.5 gap-y-1 mt-1.5 text-xs ${
              ...
              >
                ...
                {displayAddress && (
                  <>
                    ...
                    <a
                      href={
                        chain
                          ? `${chain.explorer}/address/${displayAddress}`
                          : "#"
                      }
                      target="_blank"
                    ...
                    {isOwner && (
                      <>
                        ...
                        <div className="relative" ref={ownerMenuRef}>
                          ...
                          {showOwnerMenu && (
                            <div
                              className={`absolute top-full ${menuAlign} mt-1 w-48 py-1 shadow-lg z-50 ${
                                isDark
                                  ? "bg-juice-dark border border-white/20"
                                  : "bg-white border border-gray-200"
                              }`}
                            ...
                            >
                              {OWNER_ACTIONS.filter(
                                (sec) => sec.show !== false,
                              ).map((sec, i) => (
                                <Fragment key={sec.section}>
                                  <div
                                    className={`px-3 py-1 ${i > 0 ? "mt-1 " : ""}text-[10px] font-semibold uppercase tracking-wider${i > 0 ? " border-t" : ""} ${
                                      i > 0
                                        ? isDark
                                  ...
                                  {sec.items
                                    ...
                                    .map((it) => (
                                      <button
                                        key={it.modal}
                                        onClick={() => setActiveModal(it.modal)}
                ...
                {project.metadata?.infoUri && (
                  <>
                    ...
                    <a
                      href={
                        project.metadata.infoUri.startsWith("http")
                          ? project.metadata.infoUri
                          : `https://${project.metadata.infoUri}`
                      }
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`hover:underline ${
  ...
  /**
   * All action modals, wired once for both layouts. Desktop centers with a max-width;
   * mobile is a bottom sheet with a top border. Only one is ever open (activeModal).
   */
  const renderModals = (mobile: boolean) => {
    if (!project) return null;
    return (
      <>
        {MODAL_SPECS.map(
              ...
              >
                <div
                  className={`relative w-full ${mobile ? "" : `${maxWidth} `}max-h-[90vh] overflow-y-auto${title ? "" : " p-4"} ${
                    mobile
                      ? isDark
                        ? "bg-juice-dark border-t border-white/10"
    ...
    return (
      ...
      >
        <div className="text-center max-w-md px-4">
          ...
          >
            Project existence could not be verified from the chain. No project
            actions are available.
          </p>
          {projectLoadError && (
            <p
              className={`mb-6 break-words text-xs ${isDark ? "text-red-300" : "text-red-700"}`}
            >
              {projectLoadError}
            </p>
          )}
  ...
  if (!isMobile) {
    return (
      <div
        className={`h-screen flex overflow-hidden ${isDark ? "bg-juice-dark" : "bg-white"}`}
      >
        {/* Left border */}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div
            className={`shrink-0 backdrop-blur-sm border-b ${
              isDark
                ? "bg-juice-dark/80 border-white/10"
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div
              className={`w-[380px] shrink-0 border-r flex flex-col ${
            ...
            <div className="flex-1 overflow-y-auto">{renderTabs(false)}</div>
          </div>

          {/* Chat dock - fixed at bottom */}
          <div
            className={`shrink-0 border-t ${isDark ? "border-white/10" : "border-gray-200"}`}
          >
            <ChatInput
  ...
  return (
    <div
      className={`min-h-screen flex flex-col ${isDark ? "bg-juice-dark" : "bg-white"}`}
    >
