...
interface PaymentSession {
  ...
  tokenSymbol: string;
  status:
    "pending" | "paying" | "completed" | "failed" | "expired" | "cancelled";
...
type PaymentStep =
  "loading" | "ready" | "auth" | "paying" | "success" | "error";
...
export default function PaymentPage() {
  ...
  const [pendingWalletTxHash, setPendingWalletTxHash] = useState<Hex | null>(
    null,
  );

  useEffect(() => {
    if (!sessionId) return;
    const activity = transactions.find(
      (transaction) => transaction.sessionId === sessionId,
    );
    if (
      activity?.hash &&
      (activity.status === "submitted" || activity.status === "confirmed")
    ) {
      ...
      try {
        if (pendingWalletTxHash) {
          const confirmRes = await fetch(
            `${API_BASE}/terminal/session/${sessionId}/pay/wallet/confirm`,
            {
              method: "POST",
        ...
        const res = await fetch(
          `${API_BASE}/terminal/session/${sessionId}/status`,
        );
  ...
  const handlePayWithJuice = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/terminal/session/${session.id}/pay/juice`,
        {
          method: "POST",
          headers: {
  ...
  const handlePayWithWallet = useCallback(async () => {
    ...
    try {
      ...
      const paramsRes = await fetch(
        `${API_BASE}/terminal/session/${session.id}/pay/wallet`,
      );
      ...
      const isNativeToken =
        paymentToken.toLowerCase() === NATIVE_TOKEN.toLowerCase();
      const canonicalUsdc = USDC_ADDRESSES[session.chainId as SupportedChainId];
      if (
        !isNativeToken &&
        paymentToken.toLowerCase() !== canonicalUsdc?.toLowerCase()
      ) {
      ...
      const fetchTrustedPreview = async () => {
        ...
        if (
          discoveredTerminal.address.toLowerCase() !== terminal.toLowerCase()
        ) {
          throw new Error(
            "The project payment terminal changed; request a new quote",
          );
        }
      ...
      if (lacksNativeFunds) {
        throw new Error(
          isNativeToken
            ? safeInfo
              ? "The Safe has insufficient ETH for this payment"
              : "Insufficient ETH balance and gas"
      ...
      if (safeInfo) {
        ...
        if (
          ...
          throw new Error(
            "The connected Safe or its network changed; review the payment again",
          );
        }
      }

      const hookFingerprint = (preview: typeof reviewed.preview) =>
        preview[3]
          .map((specification) =>
            [
      ...
      if (duplicate)
        throw new Error(
          "This exact merchant payment is already pending. Do not pay again.",
        );
      ...
      const startRes = await fetch(
        `${API_BASE}/terminal/session/${session.id}/pay/wallet/start`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!startData.success)
        throw new Error(
          startData.error || "Payment session is no longer available",
        );
      setStep("paying");
      ...
      if (safeInfo) {
        const txs: SafeTx[] = [];
        if (approvalRequired)
          txs.push(encodeApprovalTx(paymentToken, terminal, amount));
        txs.push({
          to: terminal,
          data: payData,
      ...
      void fetch(
        `${API_BASE}/terminal/session/${session.id}/pay/wallet/confirm`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!confirmData.success)
        throw new Error(
          confirmData.error || "On-chain payment verification is delayed",
        );
      ...
      if (activityId && !submittedTxHash) {
        ...
        useTransactionStore.getState().updateTransaction(activityId, {
          status: /rejected|denied|cancelled/i.test(message)
            ? "cancelled"
            : "failed",
          stage: undefined,
      ...
      if (submittedTxHash && !transactionReverted) {
        setError(
          "Your transaction was submitted. Do not pay again while on-chain verification is still processing.",
        );
        setStep("paying");
        return;
      }
      ...
      await fetch(
        `${API_BASE}/terminal/session/${session.id}/pay/wallet/reset`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  if (step === "loading") {
    return (
      <div
        className={`min-h-screen flex items-center justify-center ${isDark ? "bg-juice-dark" : "bg-gray-50"}`}
      >
  ...
  if (step === "error") {
    return (
      <div
        className={`min-h-screen flex items-center justify-center p-4 ${isDark ? "bg-juice-dark" : "bg-gray-50"}`}
      >
        <div
          className={`w-full max-w-sm text-center p-6 border ${isDark ? "bg-juice-dark-lighter border-white/10" : "bg-white border-gray-200"}`}
        >
          <div
            className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center ${isDark ? "bg-red-500/20" : "bg-red-100"}`}
          >
            <svg
  ...
  if (step === "success" && session) {
    return (
      ...
      >
        ...
        >
          <div
            className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-green-500`}
          >
            <svg
  ...
  if (step === "auth") {
    return (
      ...
      >
        <div
          className={`w-full max-w-sm p-6 border ${isDark ? "bg-juice-dark-lighter border-white/10" : "bg-white border-gray-200"}`}
        >
          <h1
            className={`text-lg font-semibold mb-4 text-center ${isDark ? "text-white" : "text-gray-900"}`}
          >
            Sign in to Pay
          </h1>
          ...
          {error && (
            <div
              className={`mt-4 px-3 py-2 text-xs border ${isDark ? "bg-red-500/10 border-red-500/20 text-red-400" : "bg-red-50 border-red-200 text-red-600"}`}
            >
              {error}
            </div>
          )}
          ...
          <div
            className={`mt-6 pt-4 border-t ${isDark ? "border-white/10" : "border-gray-100"}`}
          >
            <button
  ...
  return (
    <div
      className={`min-h-screen flex flex-col ${isDark ? "bg-juice-dark" : "bg-gray-50"}`}
    >
      {/* Header */}
      <header
        className={`px-4 py-3 border-b ${isDark ? "border-white/10" : "border-gray-200"}`}
      >
        <div className="max-w-sm mx-auto flex items-center justify-between">
          ...
          {timeLeft !== null && (
            <span
              className={`text-xs font-mono ${timeLeft < 60 ? "text-red-400" : isDark ? "text-gray-400" : "text-gray-500"}`}
            >
              {formatTime(timeLeft)}
            </span>
          )}
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        {session && (
          <div
            className={`w-full max-w-sm border ${isDark ? "bg-juice-dark-lighter border-white/10" : "bg-white border-gray-200"}`}
          >
            {/* Payment amount */}
            ...
            {/* Payment options */}
            <div
              className={`p-4 space-y-3 border-t ${isDark ? "border-white/10" : "border-gray-100"}`}
            >
              {/* Pay with Juice Credits */}
              {isAuthenticated() && (
                <Button
            ...
            {error && (
              <div
                className={`mx-4 mb-4 px-3 py-2 text-xs border ${isDark ? "bg-red-500/10 border-red-500/20 text-red-400" : "bg-red-50 border-red-200 text-red-600"}`}
              >
                {error}
              </div>
            )}
      ...
      {/* Footer */}
      <footer
        className={`px-4 py-3 text-center ${isDark ? "text-gray-500" : "text-gray-400"}`}
      >
        <p className="text-xs">
          Powered by{" "}
          <a href="https://juicebox.money" className="hover:underline">
            Juicebox
          </a>
        </p>
      </footer>
