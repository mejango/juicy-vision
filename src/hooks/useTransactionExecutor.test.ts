...
const { mockQuoteDirectBuy } = vi.hoisted(() => ({
  mockQuoteDirectBuy: vi.fn(),
}));
vi.mock("../services/ammMarket", () => ({
  quoteDirectBuy: (...args: unknown[]) => mockQuoteDirectBuy(...args),
}));
...
vi.mock("./useManagedWallet", () => ({
  ...
  executeManagedTransaction: (...args: unknown[]) =>
    mockExecuteManagedTransaction(...args),
}));
...
vi.mock("viem", async (importOriginal) => ({
  ...
  createPublicClient: vi.fn(() => ({
    readContract: (args: { functionName?: string }) => {
      ...
      if (args.functionName === "balanceOf")
        return Promise.resolve(mockUsdcBalance);
  ...
  concat: vi.fn(
    (...args: unknown[]) => "0x" + args.join("").replace(/0x/g, ""),
  ),
  erc20Abi: [],
  mainnet: { id: 1 },
  optimism: { id: 10 },
...
vi.mock("../api/transactions", () => ({
  createTransactionRecord: (...args: unknown[]) =>
    mockCreateTransactionRecord(...args),
  updateTransactionRecord: (...args: unknown[]) =>
    mockUpdateTransactionRecord(...args),
}));
...
const autoApproveReview = (event: Event) => {
  const request = (
    event as CustomEvent<{ respond: (approved: boolean) => void }>
  ).detail;
...
describe("useTransactionExecutor", () => {
  beforeEach(() => {
    ...
    mockQuoteDirectBuy.mockResolvedValue(null);
    ...
    mockFetch.mockResolvedValue({
      json: () =>
        Promise.resolve({ success: true, data: { spendId: "spend-123" } }),
    });
  });

  afterEach(() => {
    window.removeEventListener(
      "juice:payment-review-request",
      autoApproveReview,
    );
  });
  ...
  describe("ETH payment", () => {
    ...
    it("re-quotes and executes a requested direct swap when its minimum beats pay", async () => {
      mockQuoteDirectBuy.mockResolvedValue({
      ...
      const captureReview = (event: Event) => {
        reviewedRoute = (event as CustomEvent<{ review: { route: string } }>)
          .detail.review.route;
      };
      ...
      expect(reviewedRoute).toBe("direct pool swap");
      expect(mockQuoteDirectBuy).toHaveBeenCalledTimes(2);
      expect(mockSendTransaction).toHaveBeenCalledWith(
        expect.objectContaining({
          to: expect.not.stringMatching(/^0x130f5dd2/i),
          value: 100_000_000_000_000_000n,
        }),
      );
    });

    it("reviews and executes the Permit2 step for a USDC direct swap", async () => {
      mockQuoteDirectBuy.mockResolvedValue({
      ...
      const sufficientErc20Allowance = 2_000_000n;
      mockReadContract
        .mockResolvedValueOnce(sufficientErc20Allowance)
        .mockResolvedValueOnce([0n, 0, 0])
        .mockResolvedValueOnce(sufficientErc20Allowance)
        .mockResolvedValueOnce([0n, 0, 0]);
      let reviewedExtraCalls:
        | Array<{ label: string; contractName: string }>
        | undefined;
      const captureReview = (event: Event) => {
        reviewedExtraCalls = (
          event as CustomEvent<{
            review: {
              extraCalls: Array<{ label: string; contractName: string }>;
            };
          }>
        ).detail.review.extraCalls;
      };
      window.addEventListener("juice:payment-review-request", captureReview);
      renderHook(() => useTransactionExecutor());
      ...
      window.removeEventListener("juice:payment-review-request", captureReview);
      expect(reviewedExtraCalls).toEqual([
        expect.objectContaining({
    ...
    it("locks the reviewed preview and refuses a changed quote", async () => {
      ...
      await waitFor(() =>
        expect(mockUpdateTransaction).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            status: "failed",
            error: "The payment quote changed. Review the payment again.",
          }),
        ),
      );
      expect(mockSendTransaction).not.toHaveBeenCalled();
    ...
    it("blocks when the live USDC balance is insufficient", async () => {
      ...
      await waitFor(() =>
        expect(mockUpdateTransaction).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            status: "failed",
            error: "Insufficient USDC for this payment",
          }),
        ),
      );
      expect(mockSendTransaction).not.toHaveBeenCalled();
