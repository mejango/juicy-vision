...
export function useTransactionExecutor() {
  ...
  const executePayTransaction = useCallback(async (detail: PayEventDetail) => {
    ...
    try {
      ...
      updateTransaction(txId, { status: 'failed', error: message.slice(0, 100) })
      return
    }
    ...
    try {
      ...
      const assertSufficientLiveBalances = async () => {
        const nativeBalance = await publicClient.getBalance({ address: currentAddress })
        if (!isUsdc && nativeBalance <= projectAmount) {
          throw new Error('Insufficient ETH for the payment and network fee')
        }
        if (isUsdc) {
      ...
      const selectionFingerprint = (selection: VerifiedNFTPaymentSelection) => JSON.stringify({
      ...
      const reviewedTokens = reviewedPreview[1]
      if (reviewedTokens === 0n && !verifiedNFTSelection) {
        throw new Error('The payment quote returns no project tokens')
      }
      const quotedMinimum = (reviewedTokens * 99n) / 100n
      const reviewedMinimum = reviewedTokens === 0n
        ? 0n
        : quotedMinimum > 0n ? quotedMinimum : 1n
      ...
      const review: PaymentReview = {
        ...
        nfts: verifiedNFTSelection?.items.map(item => {
          const reviewedLabel = detail.tierSelections?.find(selection =>
            selection.tierId === item.tierId && selection.quantity === item.quantity
          )?.name.trim()
          return {
            tierId: item.tierId,
      ...
      if (
        !client?.account ||
        client.account.address.toLowerCase() !== currentAddress.toLowerCase() ||
  ...
  }, [switchChainAsync, updateTransaction, buildPayCallData, authToken])
