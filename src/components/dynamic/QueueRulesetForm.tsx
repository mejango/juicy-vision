...
function formStateToRulesetConfig(
  ...
  const metadata: JBRulesetMetadataConfig = {
    ...
    allowAddAccountingContext:
      existingConfig?.allowAddAccountingContext ?? false,
    ...
    scopeCashOutsToLocalBalances:
      existingConfig?.scopeCashOutsToLocalBalances ?? false,
    useDataHookForPay:
      dataHookConfig?.useDataHookForPay ??
      existingConfig?.useDataHookForPay ??
      false,
    useDataHookForCashOut:
      dataHookConfig?.useDataHookForCashOut ??
      existingConfig?.useDataHookForCashOut ??
      false,
    dataHook:
      dataHookConfig?.dataHook ?? existingConfig?.dataHook ?? ZERO_ADDRESS,
    metadata: existingConfig?.metadata ?? 0,
  };

  const fundAccessLimitGroups: JBFundAccessLimitGroupConfig[] =
    existingFundAccessLimitGroups.map((group) => ({
      terminal: group.terminal,
      token: group.token,
      payoutLimits: group.payoutLimits.map((limit) => ({ ...limit })),
      surplusAllowances: group.surplusAllowances.map(
  ...
  let targetGroupIndex = existingLimits
    ? fundAccessLimitGroups.findIndex(
        (group) =>
          group.terminal.toLowerCase() ===
            existingLimits.terminal.toLowerCase() &&
  ...
  if (
    targetGroupIndex === -1 &&
    (state.payoutLimitType !== "none" || state.surplusAllowanceType !== "none")
  ) {
    if (targetContext) {
  ...
  if (targetGroupIndex !== -1) {
    ...
    if (tokenDecimals === undefined)
      throw new Error(
        `Accounting token decimals unavailable on chain ${chainId}`,
      );
    const payoutCurrency =
      existingLimits?.payoutLimits[0]?.currency ?? baseCurrency;
    const allowanceCurrency =
      existingLimits?.surplusAllowances[0]?.currency ?? baseCurrency;
        ...
        : [
            {
              amount:
                type === "unlimited"
                  ? UINT224_MAX
                  : parseUnits(value || "0", tokenDecimals).toString(),
              currency,
            },
          ];
    ...
    group.surplusAllowances = [
      ...nextPrimaryLimit(
        state.surplusAllowanceType,
        state.surplusAllowance,
        allowanceCurrency,
      ),
    ...
    if (
      group.payoutLimits.length === 0 &&
      group.surplusAllowances.length === 0
    ) {
...
export default function QueueRulesetForm({
  ...
  const { state: persistedState, updateState: updatePersistedState } =
    useQueueRulesetFormState(messageId);

  // Check if form is locked (already submitted)
  const isLocked =
    persistedState?.status && persistedState.status !== "pending";
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const rulesetDataPromises = chainResolution.chains.map(
          async (chain): Promise<ChainRulesetData> => {
            try {
              ...
              if (chainProject?.currentRuleset?.id) {
                ...
                configurationComplete =
                  splitsData.configurationComplete === true;
                if (configurationComplete) {
                  assertSimpleStoredSplitGroups(splitGroups, {
                  ...
                  await assertRulesetConfigurationTrusted({
                    ...
                    approvalHook:
                      chainProject.currentRuleset.approvalHook || ZERO_ADDRESS,
              ...
              return {
                ...
                error: !chainProject?.currentRuleset
                  ...
                  : !configurationComplete
                    ? "Could not verify the complete on-chain configuration"
                    : undefined,
                selected:
                  chain.chainId === primaryChainId &&
              ...
              console.error(
                `Failed to fetch ruleset data for chain ${chain.chainId}:`,
                err,
              );
              return {
                chainId: chain.chainId,
                ...
                error:
                  err instanceof Error
                    ? err.message
                    : "Could not load the on-chain ruleset configuration",
                selected: false,
              };
            }
          },
        ...
        if (firstRuleset) {
          ...
          setFormState((prev) => ({
            ...
            weight: formatEther(BigInt(firstRuleset.weight)).replace(
              /\.?0+$/,
              "",
            ),
            weightCutPercent: (
              (firstRuleset.weightCutPercent || 0) / 10000000
            ).toString(),
        ...
        const firstLimits = allRulesetData.find(
          (cd) => cd.fundAccessLimits,
        )?.fundAccessLimits;
        if (firstLimits) {
          // Get first payout limit and surplus allowance from arrays
          const payoutLimitAmount = firstLimits.payoutLimits[0]?.amount;
          const surplusAllowanceAmount =
            firstLimits.surplusAllowances[0]?.amount;

          const payoutLimit = payoutLimitAmount
            ? BigInt(payoutLimitAmount)
            : 0n;
          const surplusAllowance = surplusAllowanceAmount
            ? BigInt(surplusAllowanceAmount)
            : 0n;
          ...
          setFormState((prev) => ({
            ...
            payoutLimitType:
              payoutLimit === 0n
                ? "none"
                : payoutLimit > unlimitedThreshold
                  ? "unlimited"
                  : "limited",
            payoutLimit:
              payoutLimit > 0n && payoutLimit < unlimitedThreshold
                ? formatUnits(payoutLimit, limitDecimals)
                : "0",
            surplusAllowanceType:
              surplusAllowance === 0n
                ? "none"
                : surplusAllowance > unlimitedThreshold
                  ? "unlimited"
                  : "limited",
            surplusAllowance:
              surplusAllowance > 0n && surplusAllowance < unlimitedThreshold
                ? formatUnits(surplusAllowance, limitDecimals)
                : "0",
          }));
        }
      } catch (err) {
  ...
  const handleConfirmed = useCallback(
    (txHashes: Record<number, string>, bundleId?: string) => {
  ...
  const changesFundLimits =
    formState.payoutLimitType !== "none" ||
    formState.surplusAllowanceType !== "none";
  const fundLimitTargetUnavailable =
    changesFundLimits &&
    selectedChains.some(
      (chainData) =>
        !chainData.fundAccessLimits &&
        chainData.accountingContexts.length !== 1,
    );
  let rulesetConfigError: string | null = null;
  let rulesetConfigsByChain: Record<number, JBRulesetConfig> = {};
  try {
    rulesetConfigsByChain = Object.fromEntries(
      chainRulesetData
        .filter(
        ...
        .map((chainData) => {
          const config = formStateToRulesetConfig(
            ...
            chainData.accountingContexts.length === 1
              ? chainData.accountingContexts[0]
              : undefined,
            {
          ...
          assertRulesetConfigurationSafe(
            chainData.chainId,
            config,
            chainData.accountingContexts,
          );
          return [chainData.chainId, config];
        }),
    );
    if (formState.memo.length > 280)
      rulesetConfigError = "Memo must be 280 characters or fewer";
  } catch (err) {
    rulesetConfigError =
      err instanceof Error ? err.message : "Ruleset configuration is invalid";
  }
  ...
  if (loading) {
    return (
      <div className="w-full">
        <div
          className={`max-w-lg border p-4 animate-pulse ${
            isDark
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-4">
          {logoUrl ? (
            <IpfsImage
              ...
              fallback={<div className="w-14 h-14 bg-purple-500/20" />}
            />
          ) : (
        ...
        {selectedChains.length > 0 && (
          <div
            className={`p-3 mb-4 ${isDark ? "bg-purple-500/10" : "bg-purple-50"}`}
          >
            <div
        ...
        <div className={`p-3 mb-3 ${isDark ? "bg-white/5" : "bg-gray-50"}`}>
          ...
          <div className="mt-3">
            ...
            <input
              ...
              onChange={(e) =>
                updateFormState("weightCutPercent", e.target.value)
              }
              placeholder="0"
              className={`w-full px-3 py-2 text-sm outline-none ${
                isDark
        ...
        <div className={`p-3 mb-3 ${isDark ? "bg-white/5" : "bg-gray-50"}`}>
          ...
          {fundLimitTargetUnavailable && (
            <div
              className={`mt-3 text-xs ${isDark ? "text-red-300" : "text-red-600"}`}
            >
              Fund limits cannot be changed here because the project does not
              have one unambiguous recognized accounting context.
            </div>
          )}
        </div>
        ...
        {isLocked && (
          ...
          >
            ...
            {persistedState?.txHashes &&
              Object.keys(persistedState.txHashes).length > 0 && (
                <div className="mt-2 space-y-1">
                  {Object.entries(persistedState.txHashes).map(
                    ([cid, hash]) => {
        ...
        {chainRulesetData.some((cd) => !cd.configurationComplete) && (
          <div
            className={`mb-3 p-3 text-xs ${isDark ? "bg-red-500/10 text-red-300" : "bg-red-50 text-red-700"}`}
          >
            Chains marked unavailable cannot be selected because their current
            on-chain configuration could not be verified.
          </div>
        )}
        {rulesetConfigError && (
          <div
