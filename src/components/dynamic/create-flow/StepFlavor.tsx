...
import { CHAIN_PAIRS, chainName, customAccounting, sanitizeState, saveDraft, stepsFor, tickerLabel } from './state'
...
function ChainPill(props: { id: number; selected: boolean; onClick: () => void }) {
  ...
  return (
    <FieldBlock label={null}>
      ...
      {state.chainIds.length > 1 ? (
            ...
            <>
              <div className="flex items-center gap-2 mt-3 text-sm">
                ...
                <Select
                  ...
                  onChange={(v) => update((s) => { s.suckerType = v as SuckerType })}
                  options={[['native', 'Native bridges'], ['ccip', 'CCIP'], ['both', 'Native and CCIP']]}
                />
              </div>
              <Hint>
                Native bridges connect Ethereum with L2s (strongest guarantees). CCIP (Chainlink) connects any
                chains. Native and CCIP gives the broadest coverage.
              </Hint>
            </>
          )}
        ...
        <InfoNote>
          {'Deploys on a single chain. ' + (state.projectType === 'revnet'
...
function CustomTokenBlock({ state, update }: StepProps) {
  ...
  useEffect(() => {
    ...
    const chainIds = chainKey.split(',').map(Number).filter((n) => Number.isFinite(n) && n > 0)
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        results = await Promise.all(chainIds.map(async (cid): Promise<ChainLookupResult> => {
          try {
            const pub = getSafetyPublicClient(cid)
            const [name, symbol, decimals] = await Promise.all([
              pub.readContract({ address: addr as `0x${string}`, abi: erc20Abi, functionName: 'name' })
                .catch(() => null),
              pub.readContract({ address: addr as `0x${string}`, abi: erc20Abi, functionName: 'symbol' }),
        ...
        if (cancelled) return
        update((s) => {
          if (s.customToken.address.trim().toLowerCase() !== addr.toLowerCase()) return
          s.customToken.status = 'error'
          s.customToken.error = 'Could not look up the token (RPC error). Try again.'
        })
        return
      }
      if (cancelled) return
      update((s) => {
        ...
        const bad = results.filter((r) => !r.ok || r.symbol !== first.symbol || Number(r.decimals) !== Number(first.decimals))
        if (bad.length) {
          t.status = 'error'
          t.error = 'Not the same token on '
            + bad.map((r) => chainName(r.cid) + (r.ok ? ` (${r.symbol}, ${r.decimals} dec)` : ' — not found')).join('; ')
    ...
    <div className="mt-2.5">
      ...
      {ct.status === 'ok' && (
        <div className="text-xs mt-2 text-teal-400">
          {(ct.name ? ct.name + ' — ' : '') + ct.symbol + ' | ' + ct.decimals + ' decimals'
...
function AccountingBlock({ state, update }: StepProps) {
  ...
  const toggleAccept = (key: AcceptKind) => {
    update((s) => {
      ...
      s.accepts = (['eth', 'usdc'] as AcceptKind[]).filter((k) => a.includes(k)) // canonical order
      applyAccountingDefaults(s)
    })
  }
      ...
      {isCustom ? (
        ...
        <>
          ...
          {isRev && state.accepts.length > 1 && (
            <PinkNote>
              Your revnet’s token will be backed by both ETH and USDC paid in by users. Holders can cash out for
              either, and the backing mix is set entirely by the proportion of payments received in each — you
          ...
          {!isRev && (
            <Hint>
              ...
              <InlineToggleLink
                label={state.swapRouter ? 'Make payers pay in your accounting token' : 'Allow payers to pay in any token'}
                onClick={() => update((s) => { s.swapRouter = !s.swapRouter })}
              />
            </Hint>
          )}
          <Hint>
            {isRev ? 'Accounting contexts cannot be added or removed later.' : 'Accounting tokens cannot be removed later.'}
          </Hint>
        </>
      )}
    </FieldBlock>
...
function OwnerSection({ state, update }: StepProps) {
  const onResolved = useCallback((addr: string | null) => {
    update((s) => { s.details.ownerResolved = addr || undefined })
  }, [update])
  return (
    <FieldBlock label="Project owner">
      {!perChainAddrActive(state, 'owner') && (
        <EnsAddressInput
          ...
          onChange={(v) => update((s) => { s.details.owner = v.trim() })}
          onResolved={onResolved}
        />
      )}
      <PerChainAddrControl state={state} update={update} fieldKey="owner" />
      <InfoNote>The address that can make changes around the configured rulesets.</InfoNote>
    </FieldBlock>
  )
}
...
function OperatorSection({ state, update }: StepProps) {
  const onResolved = useCallback((addr: string | null) => {
    update((s) => { s.revOperatorResolved = addr || undefined })
  }, [update])
  return (
    <FieldBlock label="Operator">
      {!perChainAddrActive(state, 'op') && (
        <EnsAddressInput
          ...
          onChange={(v) => update((s) => { s.revOperator = v.trim() })}
          onResolved={onResolved}
        />
      )}
      <PerChainAddrControl state={state} update={update} fieldKey="op" />
      <InfoNote>The address that operates the few controls available in revnets.</InfoNote>
    </FieldBlock>
  )
}
...
export default function StepFlavor({ state, update }: StepProps) {
  ...
  return (
    <div>
        ...
        <Select
          ...
          onChange={(v) => {
            update((s) => {
              ...
              if (v === 'revnet' && s.accepts.length > 1) s.accepts = [s.accepts[0] ?? 'eth']
      ...
      {isRev ? <OperatorSection state={state} update={update} /> : <OwnerSection state={state} update={update} />}
    </div>
  )
}
