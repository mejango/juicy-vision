...
function recomputeCustomDuration(stage: StageState): void {
  ...
  stage.durationSeconds =
    v > 0
      ? Math.round(v * (DURATION_UNIT_SECONDS[stage.customDurUnit] || 86400))
      : 0;
}
...
function fundAccessCurrencyLabel(
  currency: number,
  ...
  if (cur === 1) return "ETH";
  if (cur === 2) return "USD";
  if (customAccounting(state) && cur === customCurrencyId(state))
    return customAcctSym(state) || "TOKEN";
  return "currency " + cur;
}
...
function pkOf(
  stage: StageState,
  key: string,
): { mode: PayoutMode; recipients: RecipientRow[] } {
...
function pkEnsure(
  ...
  if (!stage.payoutByKind[key])
    stage.payoutByKind[key] = { mode: "none", recipients: [] };
...
function recipLabel(rec: RecipientRow): string {
  if (rec.type === "project" && rec.projectId)
    return "project #" + rec.projectId;
  if (rec.address)
    return rec.address.length > 10 ? truncateAddress(rec.address) : rec.address;
  return "owner";
}

function stageSummaryRaw(
  stage: StageState,
  idx: number,
  state: CreateFlowState,
): string[] {
  ...
  parts.push(
    !stage.durationSeconds
      ? "lasts until changed by project owner"
      : stage.durationSeconds === FOREVER_SECONDS
        ? "lasts forever"
        : "lasts " + secondsLabel(stage.durationSeconds),
  );
  ...
  if (stage.tokenMode === "custom") {
    parts.push(
      "issues " +
        (stage.weight || "0") +
        " tokens / " +
        fundAccessCurrencyLabel(stage.baseCurrency, state),
    );
    const reservedPct = (stage.reservedRecipients || []).reduce(
      (s, x) => s + (Number(x.percent) || 0),
      0,
    );
    if (reservedPct > 0) {
      let rLine = round2(reservedPct) + "% reserved";
      const rNamed = (stage.reservedRecipients || []).filter(
        (x) => Number(x.percent) > 0,
      );
      if (rNamed.length)
        rLine +=
          " (" +
          rNamed
            .map((x) => round2(Number(x.percent)) + "% → " + recipLabel(x))
            .join(", ") +
          ")";
      parts.push(rLine);
    }
    if (stage.issuanceCutOn && Number(stage.weightCutPercent) > 0)
      parts.push(
        "issuance cuts " +
          round2(Number(stage.weightCutPercent)) +
          "% each cycle",
      );
  ...
  if (stage.cashOutEnabled && stage.payoutMode !== "unlimited")
    parts.push(
      "cash outs on, " + round2(Number(stage.cashOutTaxRate)) + "% tax",
    );

  // Payouts — list each recipient's share.
  if (stage.payoutMode === "unlimited") {
    const uRecips = (stage.payoutRecipients || []).filter(
    ...
    const named = (stage.payoutRecipients || []).filter(
      (r) => Number(r.amountEth) > 0,
    );
    if (named.length)
      named.forEach((r) => {
        parts.push("pays " + r.amountEth + " " + pc + " → " + recipLabel(r));
      });
  ...
  if (stage.surplusAllowanceOn && stage.payoutMode !== "unlimited") {
    if (stage.surplusAllowanceUnlimited)
      parts.push("project owner can withdraw all surplus");
    else if (Number(stage.surplusAllowanceAmount) > 0)
      parts.push(
        "project owner can withdraw " +
          stage.surplusAllowanceAmount +
          " " +
          fundAccessCurrencyLabel(stage.surplusAllowanceCurrency, state) +
          " of surplus",
      );
...
function stageSummaryParts(
  stage: StageState,
  idx: number,
  state: CreateFlowState,
): string[] {
  return stageSummaryRaw(stage, idx, state).map((p) =>
...
interface StageSectionProps extends StepProps {
  stage: StageState;
  idx: number;
}
...
function SubCard({ children }: { children: ReactNode }) {
  ...
  return (
    <div
      className={`border-l-2 border-r-2 my-3.5 px-5 py-3.5 ${
        isDark
          ? "border-orange-400/40 bg-juice-dark-lighter"
...
function StageTiming({ stage, idx, state, update }: StageSectionProps) {
  ...
  const durationOptions: [string, string][] = [
    ["0", "Flexible"],
    ...DURATION_PRESETS.map((p): [string, string] => [
      String(p.seconds),
      p.label,
    ]),
  ...
  return (
    <div>
      <FieldBlock label="Duration">
        <Select
          value={
            stage.durationCustom ? "custom" : String(stage.durationSeconds)
          }
          onChange={(v) =>
            up((st) => {
              if (v === "custom") {
                st.durationCustom = true;
                recomputeCustomDuration(st);
              } else {
                st.durationCustom = false;
                st.durationSeconds = Number(v);
              }
            })
          }
        ...
        {stage.durationCustom && (
          <div className="flex items-center gap-2 mt-2">
            ...
            <Select
              ...
              onChange={(v) =>
                up((st) => {
                  st.customDurUnit = v as StageState["customDurUnit"];
                  recomputeCustomDuration(st);
                })
              }
              options={[
              ...
              <TextInput
                ...
                onChange={(v) =>
                  up((st) => {
                    ...
                    const dt = new Date(v);
                    st.schedule = isNaN(dt.getTime())
                      ? ""
                      : String(Math.floor(dt.getTime() / 1000));
                  })
                }
                className="mt-1"
              />
      ...
      <ToggleRow
        ...
        {...dz(
          "The project can receive payments.",
          "The project can’t receive payments.",
        )}
      ...
      {!isLast && !stage.durationSeconds && (
        <WarnNote>
          This stage isn’t the last, so it needs a duration. With “no expiry”,
          the next stage would start at the same moment and override this one.
          Set a duration so Stage {idx + 2} begins when this stage’s cycle ends.
        </WarnNote>
      )}
    </div>
  );
...
function TokenSection({ stage, idx, state, update }: StageSectionProps) {
  ...
  const tot =
    Math.round(
      stage.reservedRecipients.reduce(
        (s, x) => s + (Number(x.percent) || 0),
        0,
      ) * 100,
    ) / 100;
    ...
    <div>
      ...
      {stage.pausePay ? (
        <IdleToggle
          ...
          sub="No tokens are issued while payments are paused."
        />
      ) : (
        <ToggleRow
          label="Issue tokens when paid"
          {...dz(
            "Payers receive newly minted project tokens.",
            "No tokens are issued when the project is paid.",
          )}
          checked={stage.tokenMode === "custom"}
          onChange={(v) =>
            up((st) => {
              st.tokenMode = v ? "custom" : "none";
            })
          }
        />
      )}
          ...
          <div className="mt-3">
            {stage.reservedRecipients.map((rec, i) => (
              <ReservedSplitRow
                ...
                onChange={(patch) =>
                  up((st) => {
                    const r = st.reservedRecipients[i];
                    if (r) Object.assign(r, patch);
                  })
                }
                onRemove={() =>
                  up((st) => {
                    st.reservedRecipients.splice(i, 1);
                  })
                }
                state={state}
                update={update}
      ...
      <ToggleRow
        label="Give project owner privileged access to tokens"
        {...dz(
          "The project owner can mint any amount of project tokens without paying.",
          "The project owner can’t mint tokens without paying.",
        )}
        checked={stage.allowOwnerMinting}
        onChange={(v) =>
          up((st) => {
            st.allowOwnerMinting = v;
          })
        }
      />
      <ToggleRow
        ...
        {...dz(
          "Existing internal credits can’t be transferred. Claimed ERC-20 tokens remain transferable.",
          transferCopy,
        )}
  ...
  return (
    <div>
      <ToggleRow
        label="Give tokens cash out access to surplus funds"
        {...dz(
          `Token holders can cash out their tokens for a share of the project’s surplus ${label}.`,
          "Token holders can’t cash out their tokens.",
        )}
...
function PayoutKindBlock({
  stage,
  idx,
  kind,
  state,
  update,
}: StageSectionProps & { kind: PayoutKind }) {
  const pk = pkOf(stage, kind.key);
  const upPk = (
    fn: (pk: { mode: PayoutMode; recipients: RecipientRow[] }) => void,
  ) =>
    update((s) => {
      const st = s.stages[idx];
      if (st) fn(pkEnsure(st, kind.key));
    });

  const mode: "percent" | "amount" =
    pk.mode === "unlimited" ? "percent" : "amount";
  ...
  return (
    <div>
      <ToggleRow
        ...
        {...dz(
          `Routing some of the project’s ${kind.symbol} to other accounts or projects.`,
          `All ${kind.symbol} stays in the project for cash outs / later cycles.`,
        )}
        checked={pk.mode !== "none"}
        onChange={(v) =>
          upPk((p) => {
            p.mode = v ? "unlimited" : "none";
          })
        }
      />
      {pk.mode !== "none" && (
        <SubCard>
          <ToggleRow
            ...
            {...dz(
              "Paying out everything received; recipients get a percentage and the rest goes to the project owner.",
              "Paying out fixed amounts; anything else stays in the project.",
            )}
            checked={pk.mode === "unlimited"}
            onChange={(v) =>
              upPk((p) => {
                p.mode = v ? "unlimited" : "limited";
                if (!v && !p.recipients.length)
                  p.recipients.push(freshPayoutRecipient());
              })
            }
          />
          {pk.recipients.map((rec, i) => (
            <PayoutRow
              ...
              onRemove={
                mode === "amount" && pk.recipients.length <= 1
                  ? null
                  : () =>
                      upPk((p) => {
          ...
          {pk.mode === "unlimited" && (
            <Hint>
              Any {kind.symbol} not allocated above goes to the project owner.
            </Hint>
          )}
        </SubCard>
      )}
...
function PayoutsSection({ stage, idx, state, update }: StageSectionProps) {
  ...
  if (kinds) {
    const allUnlimited = kinds.every(
      (k) => pkOf(stage, k.key).mode === "unlimited",
    );
    return (
      <div>
        {kinds.map((kind) => (
        ...
        {allUnlimited ? (
          <>
            <IdleToggle
              label="Give project owner access to surplus funds"
              sub="All funds are allocated to unlimited payouts, no surplus available."
            />
            <IdleToggle
              label="Give tokens cash out access to surplus funds"
              sub="All funds are allocated to unlimited payouts, no surplus to cash out."
            />
          </>
        ) : (
          <>
            ...
            <ToggleRow
              label="Give project owner access to surplus funds"
              {...dz(
                "The project owner can withdraw the project’s surplus (funds beyond payouts) — an unlimited allowance per accepted token (ETH, USDC, …), each in its own currency.",
                "The project owner can’t withdraw from the project’s surplus.",
              )}
              checked={stage.surplusAllowanceOn}
              onChange={(v) =>
                up((st) => {
                  st.surplusAllowanceOn = v;
                  st.surplusAllowanceUnlimited = true;
                })
              }
            />
            <CashOutBlock
  ...
  // Single-token path.
  const mode: "percent" | "amount" =
    stage.payoutMode === "unlimited" ? "percent" : "amount";
  return (
    <div>
      <ToggleRow
        label="Payout funds"
        {...dz(
          "Routing some of the project’s funds to other accounts or projects.",
          "All funds stay in the project for cash outs / later stages.",
        )}
        checked={stage.payoutMode !== "none"}
        onChange={(v) =>
          up((st) => {
            st.payoutMode = v ? "unlimited" : "none";
          })
        }
      />
        ...
        <SubCard>
          <ToggleRow
            ...
            onChange={(v) =>
              up((st) => {
                ...
                if (!v && !st.payoutRecipients.length)
                  st.payoutRecipients.push(freshPayoutRecipient());
              })
            }
          />
          {stage.payoutRecipients.map((rec, i) => (
            <PayoutRow
              ...
              onChange={(patch) =>
                up((st) => {
                  const r = st.payoutRecipients[i];
                  if (r) Object.assign(r, patch);
                })
              }
              onRemove={
                mode === "amount" && stage.payoutRecipients.length <= 1
                  ? null
                  : () =>
                      up((st) => {
                        st.payoutRecipients.splice(i, 1);
                      })
              }
              state={state}
              update={update}
          ...
          {stage.payoutMode === "unlimited" && (
            <Hint>
              Any payouts not allocated above go to the project owner.
            </Hint>
          )}
        </SubCard>
      )}
      ...
      {stage.payoutMode === "unlimited" ? (
        ...
        <>
          <ToggleRow
            label="Give project owner access to surplus funds"
            {...dz(
              "The project owner can withdraw from the project’s surplus (funds beyond payouts).",
              "The project owner can’t withdraw from the project’s surplus.",
            )}
            checked={stage.surplusAllowanceOn}
            onChange={(v) =>
          ...
          {stage.surplusAllowanceOn && (
            <SubCard>
              <ToggleRow
                label="Unlimited"
                {...dz(
                  "No cap — the project owner can withdraw the entire surplus.",
                  "Capped at the amount set below.",
                )}
                checked={stage.surplusAllowanceUnlimited}
                onChange={(v) =>
                  up((st) => {
                    st.surplusAllowanceUnlimited = v;
                  })
                }
              />
              {!stage.surplusAllowanceUnlimited && (
                <div className="flex items-center gap-2 text-sm">
                  <TextInput
                    ...
                    onChange={(v) =>
                      up((st) => {
                        st.surplusAllowanceAmount = v.trim();
                      })
                    }
                    placeholder="0.0"
                    className="w-24"
...
function OtherRulesSection({
  stage,
  idx,
  update,
}: Omit<StageSectionProps, "state">) {
  const up = stageUpdater(update, idx);
  return (
    <div>
      <ToggleRow
        label="Hold fees"
        {...dz(
          "Fees are held in the project instead of processed automatically, and refunded to the project’s balance if the funds are returned. Useful for managing whole token holder refunds. After 28 days without seeing the withdrawn funds, held fees are automatically processed.",
          "Fees are processed automatically.",
        )}
      ...
      <div className="border border-pink-400/50 bg-pink-500/10 px-4 pt-3 pb-3.5 my-3.5">
        <div className="text-xs font-bold uppercase tracking-wide text-pink-400 mb-1">
          Superpowers
        </div>
        <ToggleRow
          label="Allow setting payment terminals"
          {...dz(
            "The project owner can add/remove payment terminals at any time. This lets the project upgrade, but also lets the project owner reroute where funds flow at will.",
            "Payment terminals are fixed for this ruleset.",
          )}
          checked={stage.allowSetTerminals}
          onChange={(v) =>
            up((st) => {
              st.allowSetTerminals = v;
            })
          }
        />
        <ToggleRow
          ...
          {...dz(
            "The project owner can change the project’s controller at any time. This lets the project upgrade, but can also be used by the project owner to change all the rules at will.",
            "The controller is fixed for this ruleset.",
          )}
          checked={stage.allowSetController}
          onChange={(v) =>
            up((st) => {
              st.allowSetController = v;
            })
          }
        />
        <ToggleRow
          ...
          {...dz(
            "The project owner can migrate the project’s terminals to a new version. This lets the project upgrade, but can also be used by the project owner to move all funds to a terminal of their choosing.",
            "Terminal migration is disabled.",
          )}
          checked={stage.allowTerminalMigration}
          onChange={(v) =>
            up((st) => {
              st.allowTerminalMigration = v;
            })
          }
        />
        <ToggleRow
          ...
          {...dz(
            "The project owner can replace the project’s token with a custom ERC-20 of their choosing.",
            "The project token can’t be swapped for a custom one.",
          )}
          checked={stage.allowSetCustomToken}
          onChange={(v) =>
            up((st) => {
              st.allowSetCustomToken = v;
            })
          }
        />
        <ToggleRow
          ...
          {...dz(
            "The project owner can add new accounting tokens the project holds at any time.",
            "The set of accounting tokens is fixed for this ruleset.",
          )}
          checked={stage.allowAddAccountingContext}
          onChange={(v) =>
            up((st) => {
              st.allowAddAccountingContext = v;
            })
          }
        />
        <ToggleRow
          ...
          {...dz(
            "The project owner can add price feeds the project uses to convert currencies.",
            "Price feeds can’t be added during this ruleset.",
          )}
          checked={stage.allowAddPriceFeed}
          onChange={(v) =>
            up((st) => {
              st.allowAddPriceFeed = v;
            })
          }
        />
      </div>
...
function StageCard({ stage, idx, state, update }: StageSectionProps) {
  ...
  return (
    <div
      className={`border-2 mb-3 ${isDark ? "border-orange-400/40 bg-white/[0.02]" : "border-orange-300 bg-white"}`}
    >
      <div
      ...
      >
        <div className="flex-1 min-w-0 flex flex-col gap-1">
          ...
          >
            {stageSummaryParts(stage, idx, state).map((p, i) => (
              ...
              <div key={i} className="pl-3 -indent-3">
                • {p}
              </div>
            ))}
          </div>
        ...
        <span
          className={`text-xs shrink-0 ${isDark ? "text-gray-400" : "text-gray-500"}`}
        >
          {stage.expanded ? "▾" : "▸"}
        </span>
      </div>
...
function addRuleset(s: CreateFlowState): void {
  ...
  if (prev) {
    st.tokenMode = prev.tokenMode;
    st.deadline = prev.deadline;
    st.baseCurrency = prev.baseCurrency;
    st.payoutCurrency = prev.payoutCurrency;
  } // sensible carry-over
  s.stages.forEach((x) => {
    x.expanded = false;
  });
...
function approvalPerChainHasAny(state: CreateFlowState): boolean {
  return Object.values(state.perChain || {}).some(
    (c) => !!c.addr?.["approval"],
  );
}
...
export default function StepRulesets({ state, update }: StepProps) {
  ...
  const cur: DeadlineKey =
    (state.stages[0] && state.stages[0].deadline) || "3days";
          ...
          {state.afterMode === "cycle" && (
            <>
              <ToggleRow
                ...
                {...dz(
                  "The issuance rate drops by a set amount at the end of each cycle.",
                  "The issuance rate stays the same each cycle.",
                )}
      ...
      {deadlineApplies(state) && (
        <div className="mt-6">
          <FieldBlock label="Ruleset approval condition">
            <InfoNote>
              The condition a queued edit must satisfy before it takes effect,
              giving token holders time to review changes.
            </InfoNote>
            <div className="flex items-start gap-2 flex-wrap mt-2">
              <Select
                ...
                onChange={(v) =>
                  update((s) => {
                    s.stages.forEach((st) => {
                      st.deadline = v as DeadlineKey;
                    });
                  })
                }
                options={[
                  ...DEADLINE_OPTIONS.map((o): [string, string] => [
                    o.key,
                    o.label,
                  ]),
              ...
              {cur === "custom" && !approvalPerChainHasAny(state) && (
                <EnsAddressInput
                  ...
                  onChange={(v) =>
                    update((s) => {
                      s.approvalAddress = v.trim();
                    })
                  }
                  onResolved={(addr) => {
                    const next = addr || undefined;
                    if (state.approvalResolved !== next)
                      update((s) => {
                        s.approvalResolved = next;
                      });
                  }}
                />
              )}
            ...
            {cur === "custom" && (
              <>
                ...
                <InfoNote>
                  ...
                  queued edits. Must be deployed on every selected chain.
                </InfoNote>
              </>
            )}
            {cur === "none" && (
              <WarnNote>
                No condition lets the owner make last-second edits before a
