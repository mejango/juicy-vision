...
function fundAccessCurrencyLabel(
  ...
  if (cur === 1) return "ETH";
  if (cur === 2) return "USD";
  if (state.accepts.includes("custom"))
    return state.customToken.symbol || "TOKEN";
...
export function splitTotalIssue(state: CreateFlowState): string | null {
  ...
  for (let si = 0; si < stages.length; si++) {
    ...
    if (rTot > 100.0001)
      return `${pre}reserved-token splits total ${round2(rTot)}%, over 100%.`;
    ...
    if (kinds && st.payoutByKind) {
      ...
      st.payoutMode === "unlimited" &&
      over(st.payoutRecipients) > 100.0001
    ) {
...
export function recipientIssue(state: CreateFlowState): string | null {
  const stages = state.stages || [];
  const badSplit = (
    recips: RecipientRow[] | undefined,
    kindLabel: string,
    isPayout: boolean,
  ): string | null => {
    for (const r of recips || []) {
      if (!((Number(r.percent) || 0) > 0 || (Number(r.amountEth) || 0) > 0))
        continue; // empty row — ignore
      if (r.type === "project") {
        if (!(Number(r.projectId) > 0))
          return `a ${kindLabel} to a project is missing its project ID.`;
        ...
        if (
          (!isPayout || !r.preferAddToBalance) &&
          !isAddr(pickResolved(r.address, r.resolvedAddress))
        ) {
          return `a ${kindLabel} to a project is missing the address that receives destination/fallback tokens.`;
        }
      } else if (r.type === "lphook") {
        /* fund-market hook needs no address */
      } else if (r.type === "customhook") {
        if (!isAddr(r.hookAddress))
          return `a ${kindLabel} uses a custom hook with no valid hook address.`;
  ...
  for (let si = 0; si < stages.length; si++) {
    ...
    const checks: (string | null)[] = [
      badSplit(st.reservedRecipients, "reserved-token split", false),
    ];
    if (kinds && st.payoutByKind) {
      kinds.forEach((k) => {
    ...
    for (const ai of st.autoIssuances || []) {
      if (
        (Number(ai.count) || 0) > 0 &&
        !isAddr(pickResolved(ai.address, ai.resolvedAddress))
      ) {
...
const CANON_OF: Record<number, number> = {
  11155111: 1,
  11155420: 10,
  421614: 42161,
  84532: 8453,
...
export function approvalIssue(state: CreateFlowState): string | null {
  ...
  const bad = (state.chainIds || []).filter(
    (cid) => !isAddr(chainApprovalAddr(state, cid, def)),
  );
...
function stageSummaryRaw(
  ...
  if (stage.tokenMode === "custom") {
    parts.push(
      `issues ${stage.weight || "0"} tokens / ${fundAccessCurrencyLabel(stage.baseCurrency, state)}`,
    );
    ...
    if (reservedPct > 0) {
      ...
      if (rNamed.length)
        rLine += ` (${rNamed.map((x) => `${round2(Number(x.percent))}% → ${recipLabel(x)}`).join(", ")})`;
      parts.push(rLine);
    }
    if (stage.issuanceCutOn && Number(stage.weightCutPercent) > 0) {
      parts.push(
        `issuance cuts ${round2(Number(stage.weightCutPercent))}% each cycle`,
      );
    }
  ...
  if (stage.surplusAllowanceOn && stage.payoutMode !== "unlimited") {
    if (stage.surplusAllowanceUnlimited)
      ...
      parts.push(
        `project owner can withdraw ${stage.surplusAllowanceAmount} ${fundAccessCurrencyLabel(stage.surplusAllowanceCurrency, state)} of surplus`,
      );
...
function ReviewRow({ k, v }: { k: string; v: ReactNode }) {
  ...
  return (
    <div
      className={`flex items-baseline justify-between gap-4 py-1.5 border-b last:border-b-0 ${isDark ? "border-white/5" : "border-gray-100"}`}
    >
      <span
      ...
      <span
        className={`text-sm text-right ${isDark ? "text-white" : "text-gray-900"}`}
      >
        {v}
      </span>
    </div>
...
function ReviewBullets({ k, items }: { k: string; items: string[] }) {
  ...
  return (
    <div
      className={`py-1.5 border-b last:border-b-0 ${isDark ? "border-white/5" : "border-gray-100"}`}
    >
      ...
      <div className="mt-1">
        {items.map((it, i) => (
          <div
            key={i}
            className={`text-sm pl-4 -indent-3 ${isDark ? "text-white" : "text-gray-900"}`}
          >
            • {it}
          </div>
        ))}
  ...
  return (
    <div className={box}>
      ...
      <ReviewRow
        k="Accounting token"
        v={
          surplusTokenLabel(state) +
          (state.swapRouter ? " (+ any via router)" : "")
        }
      />
      <ReviewRow
        k="Launch"
        v={
          state.stages[0] && state.stages[0].schedule
      ...
      {state.stages.map((s, i) => (
        <ReviewBullets
          key={i}
          k={`Ruleset #${i + 1}`}
          items={stageSummaryParts(s, i, state)}
        />
      ))}
      {afterApplies(state) && (
        <ReviewRow
          ...
          v={
            {
              wait: "idle (standby)",
              terminal: "continue on the same terms forever",
              cycle: "repeat the cycle forever",
            }[state.afterMode] || state.afterMode
          }
        />
      )}
      <ReviewRow
        k="Shop"
        v={
          state.shopEnabled && state.nfts.length
            ? `${state.nfts.length} item(s)`
            : "None"
        }
      />
      <ReviewRow k="Chains" v={<ReviewChains chainIds={state.chainIds} />} />
  ...
  return (
    <div>
      <StepHead
        desc={
          isRev
            ? "Review, then deploy your revnet."
            : "Review your project, then launch."
        }
      />
      ...
      {state.chainIds.length > 1 ? (
        <>
          ...
          <InfoNote>
            Each chain’s exact transaction is shown for review before you sign
            it.
          </InfoNote>
        </>
      ) : (
      ...
      {/* ToS */}
      <label
        className={`flex items-start gap-2 my-4 text-sm cursor-pointer ${isDark ? "text-gray-300" : "text-gray-700"}`}
      >
        <input
          type="checkbox"
          checked={state.tos}
      ...
      {!state.details.name && (
        <InfoNote>{`Add a project name on the Details step to ${isRev ? "deploy." : "launch."}`}</InfoNote>
      )}
      {needTicker && (
        <InfoNote>
          Add a token symbol on the Details step to deploy your revnet.
        </InfoNote>
      )}
      {needOwner && (
        <InfoNote>Set a project owner on the Flavor step to launch.</InfoNote>
      )}
      {needOperator && (
        <InfoNote>
      ...
      {needCustomToken && (
        <WarnNote>
          Your custom accounting token isn’t verified on every selected chain
          yet — fix it on the Flavor step (same address, symbol, and decimals on
          each chain).
        </WarnNote>
      )}
      {recipBad && (
      ...
      {mediaBad && (
        <WarnNote>{`${mediaBad} A store item deployed while its media is unresolved becomes a permanent name-only tier.`}</WarnNote>
      )}
      {!state.chainIds.length && (
        <WarnNote>
          Select at least one chain on the Flavor step before deploying.
        </WarnNote>
      )}
      {bad !== -1 && (
      ...
      {!state.tos && (
        <InfoNote>{`Check the box above to confirm you accept the risks before ${isRev ? "deploying." : "launching."}`}</InfoNote>
      )}

      {preparationError && (
        <div className="text-xs mt-2 text-red-400">{preparationError}</div>
      )}
    </div>
  );
}
