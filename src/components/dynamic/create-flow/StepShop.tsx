...
function storeUnit(state: CreateFlowState): string {
  return customAccounting(state)
    ? (customAcctSym(state) as string)
    : storeCur(state) === 2
      ? "USD"
      : "ETH";
}
...
function itemSummary(state: CreateFlowState, nft: ItemState): string {
  ...
  if (nft.discountOn && Number(nft.discountPct) > 0)
    parts.push(round2(Number(nft.discountPct)) + "% off");
    ...
    <div>
      ...
      <div className="mt-4">
        <Collapse
          ...
          onToggle={() =>
            update((s) => {
              s.collection.extrasOpen = !s.collection.extrasOpen;
            })
          }
        >
...
function ItemCard({
  ...
  return (
    <div
      className={`border mb-3 ${isDark ? "border-white/10 bg-white/[0.03]" : "border-gray-200 bg-gray-50/50"}`}
    >
      <div
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          <div
            className={`text-xs mt-0.5 truncate ${isDark ? "text-gray-400" : "text-gray-500"}`}
          >
      ...
      {nft.expanded && (
        <ItemEditor state={state} nft={nft} idx={idx} update={update} />
      )}
    </div>
  );
}
...
function ItemEditor({
  ...
  const onBenefResolved = useCallback(
    (addr: string | null) => setResolvedBenef(addr),
    [],
  );

  const upd = (fn: (n: ItemState) => void) =>
    update((s) => {
      const n = s.nfts[idx];
      if (n) fn(n);
    });
  ...
  const onPickMedia = (f: File) => {
    ...
    pinFileToBackend(f, nft.name || f.name)
      .then((uri) => {
        upd((n) => {
          n.imageUri = uri;
          n.mediaType = mediaTypeForFile(f);
          n._mediaUploading = false;
          n._mediaError = "";
        });
      })
      .catch((e: unknown) => {
  ...
  return (
    <div className="px-4 pb-4">
      ...
      <FieldBlock label="Media">
        ...
        {nft._mediaError && (
          <p className="text-xs text-red-400 mt-1">{nft._mediaError}</p>
        )}
        <Hint>
          Image, gif, video, audio, PDF, text… up to {ITEM_MAX_MEDIA_MB} MB.
        </Hint>
      </FieldBlock>
      ...
      {!hasPrice ? (
        <>
          <IdleToggle
            label="Split sales"
            sub="Set a price above to split each sale."
          />
          <IdleToggle
            label="Initial discount"
            sub="Set a price above to start the item at a discount."
          />
        </>
      ) : (
        <>
          <ToggleRow
            ...
            onChange={(v) =>
              upd((n) => {
                n.splitOn = v;
                if (v && !n.splitRecipients.length)
                  n.splitRecipients.push({ pct: "", recip: "", benef: "" });
              })
            }
          />
          {nft.splitOn && (
            <div className="pl-5 mb-3">
              ...
              {nft.flags.allowCredits && (
                <WarnNote>
                  Buying with shop credit bypasses this split — the split only
                  divides new payment, and a credit purchase brings in little or
                  none. To make every sale honor the split, turn off “Allow
                  credit purchases” under Extra options.
                </WarnNote>
              )}
            </div>
          )}
      ...
      <FieldBlock label="Category">
        <div className="text-xs mb-2 text-gray-500">
          Items being sold can be organized by category.
        </div>
        {catAdding ? (
          ...
          <Select
            ...
            options={[
              ["0", "Default"],
              ...state.storeCategories.map((x): [string, string] => [
                String(x.id),
                `${x.name} (#${x.id})`,
              ]),
      ...
      >
        ...
        {nft.reserveOn && (
          <>
            ...
            {benefWarn && (
              <WarnNote>
                Add the address that receives the reserved set-aside, or this
                item will fail to deploy.
              </WarnNote>
            )}
          </>
        )}
        ...
        <ToggleRow
          ...
          onChange={(v) =>
            upd((n) => {
              n.flags.ownerCanEditDiscount = v;
            })
          }
        />
        <ToggleRow
        ...
        {nft.votingOn && (
          <FieldBlock label="Voting units">
            <TextInput
              value={nft.votingUnits}
              placeholder="0"
              onChange={(v) =>
                upd((n) => {
                  n.votingUnits = v.trim();
                })
              }
            />
          </FieldBlock>
...
function ItemSplitRow({
  ...
  const upd = (
    fn: (r: { pct: number | string; recip: string; benef: string }) => void,
  ) =>
    update((s) => {
      const r = s.nfts[nftIdx]?.splitRecipients[i];
      if (r) fn(r);
    });
    ...
    <div className={i > 0 ? "mt-3" : ""}>
      <div className="flex items-center gap-2 text-sm">
        ...
        <button
          ...
          onClick={() =>
            update((s) => {
              s.nfts[nftIdx]?.splitRecipients.splice(i, 1);
            })
          }
        >
          ✕
  ...
  return (
    <div>
      <div className="text-xs mb-3 text-gray-500">
        The project owner can set or change these anytime after launch.
      </div>
      ...
      <FieldBlock label="Collection symbol">
        <TextInput
          ...
          onChange={(v) =>
            update((s) => {
              s.collection.symbol = v;
              s.collection.symbolTouched = true;
            })
          }
        />
      </FieldBlock>
      ...
      <ToggleRow
        ...
        onChange={(v) =>
          update((s) => {
            s.collection.preventOverspending = v;
          })
        }
      />
      <ToggleRow
        ...
        onChange={(v) =>
          update((s) => {
            s.collection.noNewTiersWithReserves = v;
          })
        }
      />
      <ToggleRow
        ...
        onChange={(v) =>
          update((s) => {
            s.collection.noNewTiersWithVotes = v;
          })
        }
      />
      <ToggleRow
        ...
        onChange={(v) =>
          update((s) => {
            s.collection.noNewTiersWithOwnerMinting = v;
          })
        }
      />
      <ToggleRow
        ...
        onChange={(v) =>
          update((s) => {
            s.collection.issueTokensForSplits = v;
          })
        }
      />
        ...
        (anyTokenCashOut(state) && !c.useForRedemptions ? (
          ...
          <>
            <ToggleRow
              ...
              onChange={(v) =>
                update((s) => {
                  s.collection.useForRedemptions = v;
                })
              }
            />
            {c.useForRedemptions && (
              <InfoNote>
                Items redeem at the cash out tax you set in your rulesets.
              </InfoNote>
            )}
          </>
        ))}
      ...
      {state.projectType === "revnet" && (
        <div className="mt-4">
          ...
          <ToggleRow
            ...
            onChange={(v) =>
              update((s) => {
                s.collection.opCanAdjustTiers = v;
              })
            }
          />
          <ToggleRow
            ...
            onChange={(v) =>
              update((s) => {
                s.collection.opCanUpdateMetadata = v;
              })
            }
          />
          <ToggleRow
          ...
          <ToggleRow
            ...
            onChange={(v) =>
              update((s) => {
                s.collection.opCanIncreaseDiscount = v;
              })
            }
          />
        </div>
