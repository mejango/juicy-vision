...
} from "../../services/projectBalanceBreakdown";
...
function TierPreviewImage({
  ...
  const sizeClass =
    size === "large"
      ...
      : size === "small"
        ...
        : "w-6 h-6";
  const spinnerSize =
    size === "large" ? "w-6 h-6" : size === "small" ? "w-2.5 h-2.5" : "w-3 h-3";
      ...
      .then(async (dataUri) => {
        if (dataUri) {
          try {
            ...
            if (
              onMetadataLoaded &&
              (metadata.productName || metadata.categoryName)
            ) {
  ...
  if (loading) {
    return (
      <div
        className={`${sizeClass} flex items-center justify-center ${isDark ? "bg-white/10" : "bg-gray-100"}`}
      >
        <div
          className={`${spinnerSize} border border-current border-t-transparent rounded-full animate-spin opacity-50`}
        />
      </div>
    );
  }
  ...
  if (hasIpfsImage && tier.imageUri) {
    return (
      <IpfsImage
        uri={tier.imageUri}
        alt=""
        className={`${sizeClass} object-contain bg-white`}
        onError={() => setIpfsUnavailable(true)}
      />
    );
  }
  ...
  return (
    <div
      className={`${sizeClass} flex items-center justify-center ${size === "large" ? "text-lg" : "text-[8px]"} ${isDark ? "bg-white/10 text-gray-500" : "bg-gray-100 text-gray-400"}`}
    >
      #{tier.tierId}
    </div>
  );
...
function payChainName(chainId: string): string {
  ...
  return (
    compactNames[chainId] ?? CHAIN_INFO[chainId]?.name ?? `Chain ${chainId}`
  );
}
...
type PayCardPreview =
  ...
  | {
      ...
      directSwapQuote: DirectBuyQuote | null;
...
function PaymentProgress({
  ...
  const explorerLink = hash
    ? (CHAINS[chainId]?.explorerTx || "https://etherscan.io/tx/") + hash
    : null;
  ...
  if (status === "confirmed") {
    return (
      <div
        className={`mt-2 p-2 text-sm ${isDark ? "bg-green-500/10" : "bg-green-50"}`}
      >
        <div
  ...
  if (status === "submitted" || stage === "confirming") {
    return (
      <div
        className={`mt-2 p-2 text-sm ${isDark ? "bg-juice-cyan/10" : "bg-cyan-50"}`}
      >
        <div
          className={`flex items-center gap-2 ${isDark ? "text-juice-cyan" : "text-cyan-600"}`}
        >
          <svg
  ...
  if (status === "cancelled" || status === "failed") {
    return (
      <div
        className={`mt-2 p-2 text-sm ${isDark ? "bg-yellow-500/10" : "bg-yellow-50"}`}
      >
        <div
        ...
        <button
          onClick={onRetry}
          className={`mt-2 ml-6 text-xs underline ${isDark ? "text-gray-300 hover:text-white" : "text-gray-600 hover:text-gray-900"}`}
        >
          Try again
        </button>
      </div>
  ...
  return (
    <div
      className={`mt-2 p-2 text-sm flex items-center gap-2 ${isDark ? "bg-juice-cyan/10 text-juice-cyan" : "bg-cyan-50 text-cyan-600"}`}
    >
      <svg
        className="w-4 h-4 flex-shrink-0 animate-spin"
        fill="none"
...
export default function ProjectCard({
  ...
  const { state: persistedPayment, updateState: updatePersistedPayment } =
    useProjectCardPaymentState(messageId);
  ...
  const [balanceBreakdown, setBalanceBreakdown] =
    useState<ProjectBalanceBreakdown | null>(null);
  ...
  const [payPreview, setPayPreview] = useState<PayCardPreview>({
    status: "idle",
  });
  ...
  const [countdownNow, setCountdownNow] = useState(() =>
  ...
  const [walletUsdcBalance, setWalletUsdcBalance] = useState<bigint | null>(
    null,
  );
  const [balanceLoading, setBalanceLoading] = useState(false);
  // Project's issued ERC20 token symbol (e.g., NANA for Bananapus)
  const [projectTokenSymbol, setProjectTokenSymbol] = useState<string | null>(
    null,
  );
  ...
  const [nftHookAddress, setNftHookAddress] = useState<`0x${string}` | null>(
    null,
  );
  ...
  const [paymentSafetyError, setPaymentSafetyError] = useState<string | null>(
    null,
  );
  ...
  const [routedConversion, setRoutedConversion] = useState<{
    landed: bigint;
    decimals: number;
    label: string;
  } | null>(null);
  ...
  const selectedTierIds = useMemo(
    () =>
      Object.entries(tierQuantities).flatMap(([id, qty]) =>
  ...
  const activePayment = activePaymentId
    ? getTransaction(activePaymentId)
    : null;
  ...
  // Pay Credits balance
  const { balance: juiceBalance, refetch: refetchJuiceBalance } =
    useJuiceBalance();
  ...
  const chainData = availableChains.find(
    (c) => c.chainId === parseInt(selectedChainId),
  );
  const currentProjectId =
    chainData?.projectId && chainData.projectId !== 0
  ...
  useEffect(() => {
    async function loadConnectedChains() {
      try {
        const resolution = await resolveProjectChains(
          projectId,
          parseInt(initialChainId),
        );
        setConnectedChains(resolution.chains);
        setChainMappingAvailable(resolution.mappingAvailable);
      } catch (err) {
        setError(
          err instanceof Error ? err.message : "Project chain is invalid",
        );
      }
    }
    loadConnectedChains();
  ...
  useEffect(() => {
    if (
      ...
      if (persistedPayment.selectedChainId)
        setSelectedChainId(persistedPayment.selectedChainId);
  ...
  useEffect(() => {
    ...
    async function loadScheduleGate() {
      try {
        ...
        const gate = await readPayScheduleGate(
          client,
  ...
  useEffect(() => {
    async function loadNFTTiers() {
      ...
      try {
        const hookAddr = await getProjectDataHook(
          currentProjectId,
        ...
        setNftSafetyError(
          err instanceof Error
            ? err.message
            : "Project hook configuration not recognized",
        );
      } finally {
        setTierQuantities({});
      }
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setFullMetadata(
          (data.metadata as IpfsProjectMetadata | undefined) ?? null,
        );
      } catch (err) {
        const errorMsg =
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(
        "juice:project-data-invalidated",
        refetchBalance,
      );
  }, [currentProjectId, selectedChainId]);
  ...
  useEffect(() => {
    ...
    const chains = (
      connectedChains.length > 0
        ...
        : [
            {
              chainId: parseInt(initialChainId),
    ...
    fetchProjectBalanceBreakdown(chains)
      .then((breakdown) => {
        if (!cancelled) setBalanceBreakdown(breakdown);
      })
      .catch(() => {
        if (!cancelled) setBalanceBreakdown(null);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    async function loadPaymentRoutes() {
      ...
      try {
        ...
        const accountingContexts = await getAccountingContexts(
        ...
        const directContexts = isManagedMode
          ? accountingContexts.filter(
              (context) => context.token.toLowerCase() === nativeLower,
            )
          : accountingContexts;
          ...
          directContexts.map(async (context) => {
            ...
            if (tokenLower === nativeLower) {
              ...
              const resolved = await client
                .readContract({
                  address: context.token as Address,
                  abi: erc20Abi,
                  functionName: "symbol",
                })
        ...
        const directlyAccepts = (token: Address) =>
          directOptions.some(
            (option) =>
              option.tokenAddress.toLowerCase() === token.toLowerCase(),
          );
        ...
        const routedOptions = (
          await Promise.all(
            routerCandidates.map(
              async (candidate): Promise<PaymentTokenOption | null> => {
                try {
        ...
        if (verified.length === 0)
          throw new Error("This project has no recognized payment route");
        ...
        if (!cancelled) {
          ...
          setSelectedToken((current) =>
            options.some((option) => option.symbol === current)
              ? current
              : options[0].symbol,
          );
        }
        ...
        if (!cancelled) {
          setPaymentSafetyError(
            err instanceof Error ? err.message : "Payment route not recognized",
          );
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const balances = (
      ...
      paymentTokenOptions.some((option) => option.symbol === balance.token),
    );
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (selectedTierIds.length > 0) {
          if (!nftHookAddress)
            throw new Error("NFT payment configuration is unavailable");
          const currentHook = await getProjectDataHook(
            currentProjectId,
            chainIdNum,
          );
          if (
            !currentHook ||
            currentHook.toLowerCase() !== nftHookAddress.toLowerCase()
          ) {
          ...
          metadata = buildNftPayMetadata(
            identity.metadataIdTarget,
            selectedTierIds,
          );
        }

        const beneficiary = (activeWalletAddress ??
          PREVIEW_BENEFICIARY) as Address;
        ...
        const zeroIssuance =
          selectedTierIds.length === 0 &&
          isVerifiedZeroIssuance(
            outcome.beneficiaryTokenCount,
            outcome.reservedTokenCount,
          );
        ...
        let directSwapQuote: DirectBuyQuote | null = null;
        if (
          selectedTierIds.length === 0 &&
  ...
  useEffect(() => {
    ...
    fetchPayCredits(
      ...
      .then((credits) => {
        if (!cancelled) setNftCredits(credits);
      })
      .catch(() => {
        if (!cancelled) setNftCredits(0n);
      })
      .finally(() => {
        if (!cancelled) setNftCreditsLoading(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const routedTerminal = await getPaymentTerminal(
          client,
          chainIdNum,
          BigInt(currentProjectId),
          routedToken,
        ...
        const routedIssuance =
          routedOutcome.beneficiaryTokenCount +
          routedOutcome.reservedTokenCount;

        const unit = 10n ** BigInt(acceptedDecimals);
        const acceptedTerminal = await getPaymentTerminal(
          client,
          chainIdNum,
          BigInt(currentProjectId),
          acceptedToken,
        ...
        const unitIssuance =
          unitOutcome.beneficiaryTokenCount + unitOutcome.reservedTokenCount;

        const landed = landedAcceptedAfterSwap({
          routedIssuance,
          unitIssuance,
          acceptedDecimals,
        });
        if (!cancelled) {
          setRoutedConversion(
            landed != null
              ? { landed, decimals: acceptedDecimals, label: acceptedLabel }
              : null,
          );
        }
      } catch {
  ...
  const requiresLivePayPreview =
    selectedToken !== "PAY_CREDITS" && !addsToBalance;
  ...
  const checkSufficientBalance = useCallback(
    (fresh?: { eth: bigint; usdc: bigint | null } | null) => {
      if (balanceLoading && !fresh)
        return { sufficient: false, reason: "loading" };
  ...
  const handleTierSelect = useCallback(
    (tier: ResolvedNFTTier) => {
      ...
      if (
        !paymentTokenOptions.some((option) => option.symbol === requiredToken)
      )
        return;
      ...
      setTierQuantities((prev) => {
        ...
        const totalPrice = Object.entries(newQuantities).reduce(
          (sum, [id, qty]) => {
  ...
  const handleTierMetadataLoaded = useCallback(
    (tierId: number, metadata: OnChainTierMetadata) => {
  ...
  useEffect(() => {
    const handleAddToCheckout = (
      e: CustomEvent<{ tierId: number; price: string; name?: string }>,
    ) => {
    ...
    window.addEventListener(
      "juice:add-to-checkout",
      handleAddToCheckout as EventListener,
    );
    return () =>
      window.removeEventListener(
        "juice:add-to-checkout",
  ...
  useEffect(() => {
    const handleAdjustQuantity = (
      e: CustomEvent<{ tierId: number; delta: number; name?: string }>,
    ) => {
    ...
    window.addEventListener(
      "juice:adjust-checkout-quantity",
      handleAdjustQuantity as EventListener,
    );
    return () =>
      window.removeEventListener(
        "juice:adjust-checkout-quantity",
  ...
  useEffect(() => {
    if (activePayment?.status === "confirmed") {
      ...
      window.dispatchEvent(
        new CustomEvent("juice:project-data-invalidated", {
          detail: {
  ...
  const renderSelectedTierList = () => {
    ...
    const selectedTiers = entries
      .map(([tierId, qty]) => ({
        tier: nftTiers.find((candidate) => candidate.tierId === Number(tierId)),
        qty,
      }))
      .filter((entry): entry is { tier: ResolvedNFTTier; qty: number } =>
        Boolean(entry.tier),
      );
    ...
    const fmt = (raw: bigint) =>
      formatPricingAmount(raw, pricingDecimals, pricingCurrency);
    const subtotal = selectedTiers.reduce(
      (sum, { tier, qty }) => sum + getEffectiveTierPrice(tier) * BigInt(qty),
      0n,
    );
    const restrictedCost = selectedTiers.reduce(
      (sum, { tier, qty }) =>
        tier.cannotBuyWithCredits
          ? sum + getEffectiveTierPrice(tier) * BigInt(qty)
          : sum,
      0n,
    );
    const breakdown = computeNftCheckout({
      subtotal,
      restrictedCost,
      credits: nftCredits,
    });
    ...
    const shortfall =
      entered != null && breakdown.due > entered ? breakdown.due - entered : 0n;
    ...
    return (
      <div className="mt-2">
        <div className="space-y-1">
          {selectedTiers.map(({ tier, qty }) => {
            ...
            return (
              <div
                key={tier.tierId}
                className={`flex items-center gap-2 ${exceedsSupply ? "text-orange-400" : ""}`}
              >
                ...
                {exceedsSupply && (
                  <span className="text-xs">
                    (only {tier.remainingSupply} left)
                  </span>
                )}
              </div>
            );
        ...
        <div
          className={`mt-2 space-y-0.5 border-t pt-2 text-xs ${isDark ? "border-white/10" : "border-gray-200"}`}
        >
          ...
          {entered != null && (
            <div
              className={`flex justify-between gap-4 ${shortfall > 0n ? "text-orange-400" : ""}`}
            >
  ...
  const renderPayOutcomePreview = () => {
    ...
    if (addsToBalance) {
      ...
      return (
        <div className={`mt-3 text-xs ${muted}`}>
          ...
          Adds to the project balance — nothing else. It does not issue project
          tokens; choose Pay to receive them.
        </div>
      );
    }
    if (selectedToken === "PAY_CREDITS" || payPreview.status === "idle")
      return null;
    if (payPreview.status === "loading") {
      return (
        <div className={`mt-3 text-xs ${muted}`} aria-live="polite">
          Checking live token quote…
        </div>
      );
    }
    if (payPreview.status === "unavailable") {
      return (
        <div
          className={`mt-3 text-xs ${isDark ? "text-amber-300" : "text-amber-700"}`}
          role="status"
        >
    ...
    const tokenSymbol =
      projectTokenSymbol ||
      project?.name.split(" ")[0].toUpperCase().slice(0, 6) ||
      "TOKENS";

    // A direct pool swap beats paying. The transaction executor re-quotes this
    // route immediately before review and again before submission.
    if (directSwapOffer) {
      return (
        <div
        ...
        >
          ...
          <div className="mt-1 flex min-w-0 flex-wrap items-center gap-2">
            <span
              className={`min-w-0 break-all text-lg font-semibold ${isDark ? "text-white" : "text-gray-900"}`}
            >
    ...
    if (payPreview.zeroIssuance) {
      return (
        <div className={`mt-3 text-xs ${muted}`} aria-live="polite">
          This project currently issues no tokens for payments — a payment
          simply funds the project.
    ...
    return (
      ...
      >
        ...
        <div className="mt-1 flex min-w-0 flex-wrap items-center gap-2">
          <button
            ...
            className={`min-w-0 break-all text-left text-lg font-semibold hover:underline ${
              isDark
                ? "text-white hover:text-juice-cyan"
                : "text-gray-900 hover:text-juice-orange"
            }`}
          >
        ...
        {payPreview.reservedTokenCount > 0n && (
          <div className={`mt-1 break-all text-xs font-medium ${muted}`}>
            Splits get {formatExactTokenEstimate(payPreview.reservedTokenCount)}{" "}
            {tokenSymbol}
          </div>
        )}
        {payPreview.route === "amm" &&
          ...
          (() => {
            ...
            const issuanceBenef =
              swapTotal > 0n
                ? (payPreview.grossIssuance * benef) / swapTotal
                : payPreview.grossIssuance;
            return (
              <div className="mt-1 text-xs">
                <div className={muted}>
                  ...
                  {poolLink ? (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`underline ${isDark ? "text-juice-cyan hover:text-white" : "text-cyan-700 hover:text-gray-900"}`}
                    >
                ...
                {payPreview.grossIssuance > 0n && (
                  <div className={`mt-0.5 ${muted}`}>
                    AMM:{" "}
                    {formatExactTokenEstimate(payPreview.minReturnedTokens)}{" "}
                    {tokenSymbol} vs. Issuance:{" "}
                    {formatExactTokenEstimate(issuanceBenef)} {tokenSymbol}
                  </div>
                )}
              </div>
            );
  ...
  const renderPayCaveat = () => {
    if (
      !requiresLivePayPreview ||
      payPreview.status !== "ready" ||
      payPreview.zeroIssuance
    )
      return null;
    return (
      <p
        className={`mt-2 text-[11px] leading-relaxed ${isDark ? "text-gray-500" : "text-gray-400"}`}
      >
        {payPreview.route === "amm"
    ...
    return (
      <div
        className={`mb-3 flex items-center justify-between gap-2 border px-3 py-2 text-sm ${
          isDark
            ? "border-juice-cyan/40 bg-juice-cyan/10 text-juice-cyan"
      ...
      >
        ...
        <span className="font-mono">
          {fmtCountdown(payStartsAt - countdownNow)}
        </span>
      </div>
    );
  };
  ...
  const renderSafetyNotices = () => {
    ...
    return (
      <>
        {nftSafetyError && (
          <div
            className={`mt-3 border p-3 text-xs ${isDark ? "border-amber-500/40 bg-amber-500/10 text-amber-300" : "border-amber-300 bg-amber-50 text-amber-800"}`}
          >
            {nftSafetyError}. Payments are unavailable until this project uses a
            recognized hook configuration.
          </div>
        )}
        {paymentSafetyError && (
          <div
            className={`mt-3 border p-3 text-xs ${isDark ? "border-red-500/40 bg-red-500/10 text-red-300" : "border-red-300 bg-red-50 text-red-800"}`}
          >
            {paymentSafetyError}. Payments are blocked until the live project
            route is recognized.
          </div>
        )}
      </>
  ...
  const renderPaymentModeAndChainSelector = () => {
    ...
    const disabledColor = isPaymentLocked
      ...
      <div className="relative mb-3 flex flex-wrap items-center gap-2 text-sm">
        ...
        <div className="relative">
          ...
          >
            ...
            {canSelectChain && (
              <svg
                className={`h-3 w-3 transition-transform ${chainDropdownOpen ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          {chainDropdownOpen && canSelectChain && (
            <div
              className={`absolute left-0 top-full z-30 mt-1 min-w-[150px] border py-1 shadow-lg ${isDark ? "border-white/10 bg-juice-dark" : "border-gray-200 bg-white"}`}
              role="menu"
            >
              {availableChains.map((chain) => {
                ...
                return (
                  ...
                  >
                    ...
                    {chain.projectId !== 0 &&
                      chain.projectId.toString() !== projectId && (
                        <span
                          className={`ml-1 text-xs ${isDark ? "text-gray-500" : "text-gray-400"}`}
                        >
                          (#{chain.projectId})
                        </span>
                      )}
  ...
  if (loading) {
    return (
      <div className="w-full">
        <div
          className={`max-w-md border p-4 animate-pulse ${isDark ? "bg-juice-dark-lighter border-gray-600" : "bg-white border-gray-300"}`}
        >
          {/* Header skeleton */}
          <div className="flex items-center gap-3 mb-2">
            <div
              className={`w-14 h-14 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
            />
            <div className="flex-1">
              <div
                className={`h-5 w-32 mb-1 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
              />
              <div
                className={`h-3 w-20 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
              />
            </div>
          </div>
          ...
          {/* Tagline skeleton */}
          <div
            className={`h-4 w-3/4 mb-3 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
          />

          {/* Stats row skeleton */}
          <div className="flex gap-6 mb-3">
            <div
              className={`h-4 w-24 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
            />
            <div
              className={`h-4 w-16 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
            />
            <div
              className={`h-4 w-20 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
            />
          </div>
          ...
          <div className={`p-3 mb-3 ${isDark ? "bg-white/5" : "bg-gray-50"}`}>
            {/* Chain selector */}
            <div
              className={`h-4 w-28 mb-3 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
            />

            {/* Amount input row */}
            <div className="flex gap-2 mb-2">
              <div
                className={`flex-1 h-10 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
              />
              <div
                className={`w-16 h-10 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
              />
            </div>
            ...
            <div className="flex gap-2 mb-2">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className={`h-6 w-12 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
                />
              ))}
            </div>
            ...
            {/* Token preview */}
            <div
              className={`h-4 w-40 mb-4 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
            />
          ...
          <div className="flex items-center gap-2 mt-2">
            <div
              className={`w-3.5 h-3.5 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
            />
            <div
              className={`h-4 w-28 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
            />
          </div>
        </div>
      </div>
  ...
  const balanceAvailable =
    !!suckerBalance && suckerBalance.balanceAvailable !== false;
  const paymentsAvailable =
    !!suckerBalance && suckerBalance.paymentsAvailable !== false;
  const displayedBalance = suckerBalance?.totalBalance ?? project.balance;
  const balanceScope =
    suckerBalance?.dataScope === "project" ? " on this chain" : " balance";

  // Calculate total balance USD value using currency/decimals from suckerBalance
  const totalBalanceUsd = balanceAvailable
    ? formatUsd(
        displayedBalance,
        suckerBalance.currency,
        suckerBalance.decimals,
      )
    : null;
  ...
  const balanceTooltipRows: Array<{
    key: string;
    label: string;
    value: string;
  }> = balanceBreakdown
    ? balanceBreakdown.rows
        .filter((row) => row.balance > 0n)
        .map((row, index) => ({
          key: `${row.chainId}-${row.token}-${index}`,
          label:
            CHAIN_INFO[row.chainId.toString()]?.name ?? `Chain ${row.chainId}`,
    ...
    : (suckerBalance?.projectBalances ?? [])
        ...
        .map((pb) => {
          ...
          return {
            ...
            value:
              pbCurrency === 2
                ? `$${formatBalance(pb.balance, pbDecimals)}`
                : `${formatBalance(pb.balance, pbDecimals)} ETH`,
          };
        });

  const showBalanceTooltipContent =
    (balanceBreakdown ? balanceBreakdown.verified : balanceAvailable) &&
    balanceTooltipRows.length > 0;
  ...
  const handlePay = async (event?: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const tierSelections = Object.entries(tierQuantities).map(
      ([tierId, quantity]) => {
        const tier = nftTiers.find(
          (candidate) => candidate.tierId === Number(tierId),
        );
        return {
    ...
    window.dispatchEvent(
      new CustomEvent("juice:pay-project", {
        detail: {
          ...
          tierSelections,
          hookAddress: nftHookAddress,
          directSwapRequested: Boolean(directSwapOffer),
        },
      }),
    );
  ...
  const renderTierCarousel = (
    ...
    return (
      <div className={wrapperClassName}>
        ...
        <div
          className={`flex gap-3 overflow-x-auto pt-3 pb-2 ${scrollPaddingClassName}`}
        ...
        >
          {nftTiers.slice(0, 12).map((tier) => {
            ...
            const currencySupported =
              currencyRecognized &&
              paymentTokenOptions.some((option) => option.symbol === tierToken);
            return (
              <div
                key={tier.tierId}
                title={
                  currencySupported
                    ? undefined
                    : currencyRecognized
                      ? `This project does not accept ${tierToken}`
                      : `Pricing currency not recognized: ${tier.currency}`
                }
              ...
              >
                ...
                <div className="p-1.5 text-left">
                  <div
                    className={`text-[10px] font-medium truncate ${isDark ? "text-white" : "text-gray-900"}`}
                  >
                    {getTierDisplayName(tier)}
                  </div>
                  <div
          ...
          {nftTiers.length > 12 && (
            <button
              onClick={() =>
                window.dispatchEvent(new CustomEvent("juice:open-shop"))
              }
        ...
        >
          ...
          <div className="relative ml-auto flex-shrink-0">
            ...
            >
              ...
              <svg
                className={`w-3 h-3 transition-transform ${tokenDropdownOpen ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            {tokenDropdownOpen && (
              <div
                className={`absolute top-full right-0 mt-1 py-1 shadow-lg z-10 min-w-[140px] ${isDark ? "bg-juice-dark border border-white/10" : "bg-white border border-gray-200"}`}
              >
  ...
  const renderMemoAndFeedback = () => (
    <>
      ...
      {(activePayment ||
        (persistedPayment && persistedPayment.status !== "pending")) && (
        <PaymentProgress
          ...
          status={
            ...
            ((persistedPayment?.status === "completed"
              ? "confirmed"
              : persistedPayment?.status === "failed"
                ? "failed"
                : "submitted") as TransactionStatus)
          }
    ...
    createPortal(
      <>
        ...
        >
          ...
          <h3
            className={`text-sm font-semibold mb-1 pr-6 ${isDark ? "text-white" : "text-gray-900"}`}
          >
          ...
          <div className="space-y-2">
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  >
                    5% markup · available after verification
                  </div>
                </div>
                <svg
            ...
            {/* Crypto option */}
            <div
              className={`w-full p-3 text-left border ${isDark ? "border-white/20" : "border-gray-200"}`}
            >
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  <div
                    className={`text-xs ${isDark ? "text-gray-400" : "text-gray-500"}`}
                  >
                    Network fees and confirmation time vary
                  </div>
                </div>
              </div>
              {managedAddress ? (
                <div className="mt-2">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`ml-2 text-xs ${copiedAddress ? "text-green-500" : isDark ? "text-gray-500" : "text-gray-400"}`}
                      >
                        {copiedAddress ? "✓" : "Copy"}
                      </span>
                    </div>
      ...
      {embedded ? (
        <>
          ...
          <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            {notStarted && (
              <div
                className={`px-4 pt-3 ${isDark ? "bg-[#222]" : "bg-gray-50"}`}
              >
            ...
            {renderTierCarousel(
              `px-4 pt-3 ${isDark ? "bg-[#222]" : "bg-gray-50"}`,
              "-mx-4 px-4",
            )}

            {/* Sticky pay controls - sticks to top when scrolling */}
            <div
              className={`sticky top-0 z-20 px-4 ${nftTiers.length > 0 ? "py-3" : "pt-4 pb-3"} ${isDark ? "bg-[#222]" : "bg-gray-50/80 backdrop-blur-sm"}`}
            >
            ...
            <div
              className={`px-4 pt-3 pb-2 border-t ${isDark ? "border-white/10" : "border-gray-200"}`}
            >
              <span
        ...
        <div className="w-full">
          ...
          <div
            className={`max-w-md border p-4 ${isDark ? "bg-juice-dark-lighter border-gray-600" : "bg-white border-gray-300"}`}
          >
                ...
                <IpfsImage
                  ...
                  fallback={
                    <div className="w-14 h-14 bg-juice-orange/20 flex items-center justify-center">
                      <span className="text-juice-orange font-bold text-xl">
            ...
            {(fullMetadata?.tagline || fullMetadata?.projectTagline) && (
              <p
                className={`text-sm italic mb-3 ${isDark ? "text-gray-400" : "text-gray-500"}`}
              >
                {fullMetadata.tagline || fullMetadata.projectTagline}
              </p>
            )}
            ...
            <div className="flex gap-6 mb-3 text-sm">
              ...
              >
                <span
                  className={`font-mono cursor-pointer ${isDark ? "text-white" : "text-gray-900"}`}
                >
                  {balanceHeadline}
                </span>
                ...
                {showBalanceTooltip && showBalanceTooltipContent && (
                  <div
                    className={`absolute top-full left-0 mt-1 p-2 shadow-lg z-20 min-w-[180px] text-xs ${isDark ? "bg-juice-dark border border-white/20" : "bg-white border border-gray-200"}`}
                  >
                    ...
                    <div
                      className={`flex justify-between gap-4 pt-1 mt-1 border-t ${isDark ? "border-white/10" : "border-gray-100"}`}
                    >
                      <span
                        className={isDark ? "text-gray-400" : "text-gray-500"}
                      >
              ...
              >
                <span
                  className={`font-mono cursor-help ${isDark ? "text-white" : "text-gray-900"}`}
                >
                  {paymentsAvailable
                ...
                <span className={isDark ? "text-gray-400" : "text-gray-500"}>
                  {" "}
                  payment
                  {!paymentsAvailable || suckerBalance.totalPaymentsCount !== 1
                    ? "s"
                    : ""}
                </span>
                ...
                {showPaymentsTooltip &&
                  paymentsAvailable &&
                  suckerBalance.projectBalances.length > 0 && (
                    <div
                      className={`absolute top-full left-0 mt-1 p-2 shadow-lg z-20 min-w-[140px] text-xs ${isDark ? "bg-juice-dark border border-white/20" : "bg-white border border-gray-200"}`}
                    >
            ...
            <div
              className={`mb-3 px-3 pb-3 ${nftTiers.length > 0 ? "pt-3" : "pt-4"} ${isDark ? "bg-white/5" : "bg-gray-50"}`}
            >
              {!chainMappingAvailable && (
                <ChainMappingWarning isDark={isDark} />
              )}
