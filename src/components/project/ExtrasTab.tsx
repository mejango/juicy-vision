...
function formatFacilitated(row: ProjectPayerRow): string {
  try {
    const usd = BigInt(
      String(row.totalFacilitatedUsd || "0").split(".")[0] || "0",
    );
    if (usd > 0n) {
    ...
    return BigInt(String(row.totalFacilitated || "0").split(".")[0] || "0") > 0n
      ? "Unpriced"
      : "$0";
  } catch {
    return "$0";
...
function countsText(row: ProjectPayerRow): string {
  ...
  if (Number(row.addToBalanceCount || 0))
...
export default function ExtrasTab({
  ...
  const chains = useMemo(() => {
    const list = (connectedChains || []).filter(
      (c) => c.chainId && c.projectId,
    );
  ...
  useEffect(() => {
    if (editable && !adminAddress && activeAddress)
      setAdminAddress(activeAddress);
  ...
  const [selectedChains, setSelectedChains] = useState<number[]>(() =>
    chains
      .filter((c) => getProjectPayerDeployer(c.chainId))
      .map((c) => c.chainId),
  );
  ...
  const {
    ...
    onSuccess: () => {
      setDeployStatus({
        kind: "success",
        text: "Payer address deployment complete via Relayr.",
      });
      setDeployBusy(false);
      loadPayers();
    },
    onError: (error) => {
      setDeployStatus({
        kind: "error",
        text: error.message || "Could not deploy the payer address.",
      });
      setDeployBusy(false);
    },
  });
  ...
  useEffect(() => {
    setSelectedChains((previous) => {
      ...
      const added = known.filter(
        (id) => !previous.includes(id) && !kept.includes(id),
      );
      return [...kept, ...added];
    });
  }, [chains]);
  ...
  const duplicates = useMemo(() => {
    ...
    if (originalPayer) formBeneficiary = "";
      ...
      formBeneficiary = isAddress(raw, { strict: false })
        ? raw.toLowerCase()
        : null; // null = can't compare yet
    }
    ...
    return payers.filter((row) => {
      ...
      const rowBeneficiary =
        row.defaultBeneficiary && !/^0x0{40}$/i.test(row.defaultBeneficiary)
  ...
  const relayrActive = useRelayr && isManagedMode && selectedChains.length > 1;
  ...
  const handleDeploy = useCallback(async () => {
    ...
    if (!activeAddress) {
      setDeployStatus({
        kind: "error",
        text: "Connect a wallet to deploy payer addresses.",
      });
      return;
    }
    setDeployBusy(true);
    try {
      ...
      for (const target of targets) {
        ...
        const resolvedBeneficiary = originalPayer
          ? zeroAddress
          : await resolveAddressInput(
              beneficiaryOverrides[cid]?.trim() || beneficiary,
              "beneficiary",
            );
        let owner: string = zeroAddress;
        if (editable) {
          owner = await resolveAddressInput(
            adminOverrides[cid]?.trim() || adminAddress || activeAddress,
            "address admin",
          );
          if (/^0x0{40}$/i.test(owner)) {
            throw new Error(
              `Editable payer addresses need a nonzero address admin on ${chainName(cid)}.`,
            );
          }
        }
        calls.push(
      ...
      if (relayrActive) {
        ...
        targets.forEach((t) => {
          projectIds[t.chainId] = t.projectId;
        });
      ...
      setDeployStatus({
        ...
        text:
          error instanceof Error
            ? error.message
            : "Could not deploy the payer address.",
      });
      setDeployBusy(false);
    }
  }, [
  ...
  const select = `select-caret pl-3 pr-6 py-2 text-sm border outline-none ${
    isDark
      ? "bg-juice-dark border-white/15 text-white"
      : "bg-white border-gray-300 text-gray-900"
  }`;
  const button =
    "px-4 py-2 text-sm font-medium bg-juice-orange text-juice-dark hover:bg-juice-orange/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed";
  ...
  const renderStatus = (status: Status) =>
    status.text ? (
      <div className={`text-sm ${statusClass(status)}`}>{status.text}</div>
    ) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={card}>
        ...
        <ExplainerMessage>
          A payer address lets anyone pay this project by simple transfer — send
          the native token (ETH) straight to it, no app needed. Sending ERC-20
          tokens directly to it does not trigger a payment. Anyone can create
          any number of payer addresses.
        </ExplainerMessage>
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            value={mode}
            onChange={(e) =>
              setMode(e.target.value === "balance" ? "balance" : "pay")
            }
          >
        ...
        <div className="space-y-1">
          ...
          {editable ? (
            <>
              ...
              />
              <div className={subText}>
                The address admin can later change this payer address’s
                destination project, Pay/Add to Balance behavior, beneficiary,
                memo, and metadata, or transfer or renounce the admin role. The
                role does not receive payments or control either project.
              </div>
            </>
          ) : (
            <div className={subText}>
              Off by default: no address admin. This payer address’s destination
              project, Pay/Add to Balance behavior, beneficiary, memo, and
              metadata are permanent.
            </div>
          )}
        </div>
        ...
        <details>
          <summary
            className={`cursor-pointer text-sm ${isDark ? "text-gray-400" : "text-gray-600"}`}
          >
            Extra options
          </summary>
          <div className="mt-2 space-y-1">
            ...
            />
            <div className={subText}>
              Hex bytes forwarded to pay/addToBalance. Leave 0x unless you need
        ...
        {duplicates.length > 0 && (
          ...
          >
            ...
            {duplicates
              .map(
                (row) =>
                  `${chainName(row.chainId)} ${truncateAddress(row.address)}`,
              )
              .join(", ")}
            . Anyone can pay it directly — deploying again creates another
            address that behaves the same.
          </div>
        )}
        ...
        {isManagedMode && (
          ...
          >
            ...
            <span className={subText}>
              one prepaid payment deploys to every selected chain
            </span>
          </label>
        )}
        ...
        <div className="pt-2 space-y-2">
          ...
          {payers === null && !payersError && (
            <div className={subText}>
              Loading payer addresses from Bendystraw…
            </div>
          )}
          {payersError && (
            <div className={subText}>
            ...
            >
              {payers.map((row) => {
                const customBeneficiary =
                  ...
                  !/^0x0{40}$/i.test(row.defaultBeneficiary)
                    ? row.defaultBeneficiary
                    : null;
                return (
                  <div
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <div className={subText}>
                        {row.lastUsedAt
                          ? `Last used ${timeAgo(row.lastUsedAt)}`
                          : "Never used"}
                        {row.createdAt
