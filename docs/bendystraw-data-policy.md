...
1. Execute a committed, static GraphQL document through
   `requestBendystraw`.
   When the optional public backend proxy is used, the browser sends only a
   SHA-256 persisted-operation ID plus variables. The backend substitutes a
   build-registered document and rejects unknown IDs, raw documents, extra
   payload fields, and implicit networks.
