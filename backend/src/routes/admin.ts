...
// Every route in this router reads sensitive operational data or mutates
// privileged state. Keep authorization at the router boundary so new routes
...
adminRouter.get(
  '/juice/pending-spends',
  zValidator('query', PendingSpendsQuerySchema),
  async (c) => {
    ...
    `,
      ...
      const spends = rows.map((row) => ({
        ...
        lastRetryAt: row.last_retry_at instanceof Date
          ? row.last_retry_at.toISOString()
          : row.last_retry_at,
        createdAt: row.created_at instanceof Date ? row.created_at.toISOString() : row.created_at,
        updatedAt: row.updated_at instanceof Date ? row.updated_at.toISOString() : row.updated_at,
      }));
