...
export async function getChatsForAddress(
  ...
  const countResult = await queryOne<{ count: string }>(
    `SELECT COUNT(DISTINCT mc.id) as count ${baseSql}`,
    params,
  );
  ...
  let sql =
    `SELECT mc.* ${baseSql} ORDER BY mc.is_pinned DESC, mc.pin_order ASC NULLS LAST, mc.updated_at DESC`;
...
export async function getChatMembers(chatId: string): Promise<ChatMember[]> {
  ...
  console.log(
    `[getChatMembers] Chat ${chatId}: Found ${results.length} active members:`,
    results.map((r) => ({ address: r.member_address, role: r.role, is_active: r.is_active })),
  );
...
export async function addMemberViaInvite(
  ...
  const permissions = {
    ...
    canManageMembers: role === 'admin' || role === 'founder'
      ? basePermissions.canManageMembers
      : false,
    canPauseAi: canPauseAi ??
      (role === 'admin' || role === 'founder' ? basePermissions.canPauseAi : false),
  };
...
export async function sendMessage(params: SendMessageParams): Promise<ChatMessage> {
  const { chatId, senderAddress, senderUserId, content, signature, replyToId, attachments } =
    params;
...
export async function addAiBalance(
  ...
  await transaction(async (client) => {
    ...
    await client.queryObject`
      ...
      VALUES (${chatId}, 'deposit', ${amountWei.toString()}, ${payerAddress}, ${txHash ?? null}, ${
      projectId ?? null
    }, ${chainId ?? null})
    `;
  });
...
export async function updateChatName(chatId: string, name: string): Promise<Chat | null> {
  await execute('UPDATE multi_chats SET name = $1, updated_at = NOW() WHERE id = $2', [
    name,
    chatId,
  ]);
